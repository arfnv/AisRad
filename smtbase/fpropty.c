...
/*  Locate the methods table for user defined types (see defclass). */
...
/*  Set the methods table for user defined types (see defclass). */
...
Return the methods of the specified class or object. 
The methods ares either void or a Structure object.
...
if (argv[0].Tag == TYLAMBDA)
	...
	/* If methods are void, then return the Lambda Pv. */
	if (ret->Tag == TYVOID)
		{
		*ret = TOBJ(argv[0].u.Lambda->PersistantVariables);
		}
...
Return the fields of the specified class or object. 
The fields are either void, or a Structure object.
...
The FProperty_Factory cProcedure is a structure (see defclass) constructor 
