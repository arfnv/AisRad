...
/* Add the child name to the parent Lambda methods list (if necessary). */
...
if ((parentLambda->Tag == TYLAMBDA) && (parentLambda->u.Lambda->Interfaces != NIL))
{
	*faces = TOBJ(parentLambda->u.Lambda->Interfaces);
	*tmp = TSYMBOL("methods");
	*methods = FSmartbase_Eval(gCP, gTP, TGVALUE("ref"), 2, *faces, *tmp);
...
if ((argPair->itsCdr.Tag == TYPAIR) &&
	...
	*classLambda = className->u.Symbol->itsGlobalValue;
	if (classLambda->Tag == TYLAMBDA)
		...
		*tmp = TSYMBOL("parent");
		*tmp = TStructure_SetIV1(gCP,gTP,*faces,*tmp,gCP->Tval_VOID);
		...
		*tmp = TStructure_SetIV1(gCP,gTP,*faces,*tmp,*macros);
		...
		/* Add class persistant child Lambdas to the class methods list. */
		*tmp = TOBJ(classLambda->u.Lambda->PersistantVariables);
		if (tmp->Tag == TYSTRUCTURE) N = tmp->u.Structure->itsMaxItemIndex; else N = 0;
		for (n = 0; n < N; ++n)
			{
			if (BindArray(*tmp)[n].Value.Tag == TYLAMBDA)
				{
				TStructure_SetIV1(gCP, gTP, *methods, TOBJ(BindArray(*tmp)[n].Key), BindArray(*tmp)[n].Value);
				}
			else
			if (BindArray(*tmp)[n].Value.Tag == TYMACRO)
				{
				TStructure_SetIV1(gCP, gTP, *macros, TOBJ(BindArray(*tmp)[n].Key), BindArray(*tmp)[n].Value);
				}
			}
		}
	FrameExit(*result);
...
if (parentLambda->Tag != TYLAMBDA)
	{
	*result = TERROR("!defclass: parent must be a Lambda!");
	FrameExit(*result);
	}

/* Locate the parent Pv Cv and Sv elements.  */
...
if (gTP->FCompile_GenerateDebugInfo == TRUE)
	{
	/* We are generating debugging information during this compile. */
	*tail = FSmartbase_Eval(gCP, gTP, TGVALUE("list"), 7, TSYMBOL("makeLambda"), TQSYMBOL("Cv"), *parentCv, TQSYMBOL("Sv"), *parentSv, TQSYMBOL("In"), gTP->FCompile_DebugInterfaces);
	ExitOnError(*tail);
	}
else
	{
	/* We are NOT generating debugging information during this compile. */
	*tail = FSmartbase_Eval(gCP, gTP, TGVALUE("list"), 5, TSYMBOL("makeLambda"), TQSYMBOL("Cv"), *parentCv, TQSYMBOL("Sv"), *parentSv);
	ExitOnError(*tail);
	}
...
/* Assign the child class to the child class name as a global variable. */

className->u.Symbol->itsGlobalValue = *classLambda;
	...
	*tmp = TStructure_SetIV1(gCP,gTP,*faces,TSYMBOL("class"),*className);
	...
	if (parentIsClassSW == TRUE)
		...
		*tmp = TStructure_SetIV1(gCP,gTP,*faces,TSYMBOL("parent"),*parentLambda);
		...
		{
		*tmp = TStructure_SetIV1(gCP,gTP,*faces,TSYMBOL("parent"),gCP->Tval_VOID);
		ExitOnError(*tmp);
		}
