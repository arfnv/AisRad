...
while( hNdx < hLen  )
    ...
    switch ((curChar = atHMChar(hSrc,hNdx++)))
        ...
        case '{':
			if ((atHMChar(hSrc,hNdx + 0) == '#') &&
				(atHMChar(hSrc,hNdx + 1) == '#') &&
				(atHMChar(hSrc,hNdx + 2) == '['))
			{
				//  AIS Lisp supports string constants delimited by {##[...string constant...]##} paired '{##[' ']##}' 
				//  special character sequences. The left brace sequence '{##[' is used as a way of indicating string constants.
				//  AIS Lisp string constants are case-sensitive, may include blanks, special characters, 
				//  but NOT imbedded right brace ']##}' character sequences.
	            hNdx += 3;
				for(anInt = 0;  ((&atHMChar(hSrc,0))[hNdx] != 0); /*hNdx < hLen*/)
				{
					/*  We parse forward through the source, looking for the end of string sequence */
					if (anInt >= stringConstMaxLen)
						{
					...
					if ((atHMChar(hSrc,hNdx + 0) == ']') &&
						(atHMChar(hSrc,hNdx + 1) == '#') &&
						(atHMChar(hSrc,hNdx + 2) == '#') &&
						(atHMChar(hSrc,hNdx + 3) == '}'))
					{
						/*  String constants are terminated with a right brace ']##}' sequence. */
	                    hNdx += 4;
						break;
					}
					else    
			...
			{
				//  AIS Lisp supports string constants delimited by {...string constant...} paired '{' '}' 
				//  special character pairs. The left brace '{' is used as a way of indicating string constants.
				//  AIS Lisp string constants are case-sensitive, may include blanks, special characters, 
				//  and BALANCED imbedded right brace '}' characters.
