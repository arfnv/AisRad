...
Locks the specified symbol OR locks all the nonempty symbols in the global symbol table.

When a symbol is locked, its locked property is set to 1, and is protected when the (clear)
function is issued.

The (clear) funcion sets all unlocked global symbols to #void and garbage collects.
...
if ((argc == 1) && (argv[0].Tag == TYSYMBOL))
{
	// Lock the specified symbol.
	sym = argv[0].u.Symbol;
	sym->itsGlobalLock = TRUE;
}
else if ((argc == 1) && (argv[0].Tag == TYGLOBALS))
{
	// Lock all global symbols whose global values are currently non-empty.
...
{
	FrameExit(TERROR("!lock: invalid argument!"));
}
