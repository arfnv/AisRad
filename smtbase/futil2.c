...
An example follows.
    ...
    (quit)		=>  ...requests an escape back to the console...
    (quit true) =>  ...requests a quit of the application...
...
if ((argc >= 1) && (argv[0].Tag == TYBOLE) && (argv[0].u.Bool == TRUE))
	gTP->escapeSW = 2;		/* Set a quit request */ 
else
	gTP->escapeSW = 1;		/* Set an escape request */
...
Note:   If no argument is passed, the amount of free memory will be returned.
		...
		If objectList: or globals: is passed, a vector of all active objects will be returned in an object vector.
		If symbolList: sort showCFunc showLocked - are passed, a vector of all symbol objects will be returned in an object vector.
		If lambdas: is passed, a vector of all active Lambda objects will be returned in an object vector.
		...
		If ObjectRepository is passed then the size of the ObjectRepository on disk is returned.
		If ObjectRepository open: is passed then true is returned if the ObjectRepository is open.
		If ObjectRepository length: key is passed then length of the item on disk is returned (or immediate: if the item is immediate).
		If ObjectRepository check: is passed then a systems check of the ObjectRepository is performed.
		If ObjectRepository free: is passed then the number of free bytes remaining in the database is returned.
		If ObjectRepository directory: is passed then a copy of the index directory for the database is returned.
		If ObjectRepository index: is passed then the index allocation statistics for the database are displayed.
		If ObjectRepository pages: is passed then the page statistics for the database are displayed.
		If ObjectRepository show: is passed then the full statistics for the database are displayed.
		If ObjectRepository stats: is passed then a Structure of ObjectRepository statistics for the database is returned.
...
if ((argc == 1) && (asTag(&argv[0]) == TYSYMBOL) && (asSymbol(&argv[0]) == TSymbol_MakeUnique(gCP,gTP,"lambdas")))
    {
	*objList = TObjVector_MakeNew(gCP,gTP,0,argv);
    ...
    for (oid = 1; oid < gCP->TObject_MaxObjectCount; ++oid)
		...
		if (_TObject_ObjectFlag(oid) != _TObject_OfVOID)
			...
			if ((object->itsObjectIndex == oid) && (object->itsObjectType == TYLAMBDA))
				{
...
if ((argc == 1) && (asTag(&argv[0]) == TYSYMBOL) && (asSymbol(&argv[0]) == TSymbol_MakeUnique(gCP,gTP,"objectList")))
    {
	ObjectList:
	*objList = TObjVector_MakeNew(gCP,gTP,0,argv);
	tmpInt = 0;
...
if ((argc == 4) && (argv[0].Tag == TYSYMBOL) && (argv[0].u.Symbol == TSymbol_MakeUnique(gCP,gTP,"symbolList")))
    {
	*objList = FDebug_GetSymbolTable(gCP,gTP,argc-1,&argv[1]);
