...
gTP->escapeSW = 0;	/* No escape has been requested */
gTP->busySW = FALSE;
...
/* Convert the real number to exponential and check for fraction present. */

exponent = log10(abs(aReal));
wholeNumberSW = (aReal == floor(aReal));

/* Convert the real number to explicit/exponential format depending on its exponent. */

if ((exponent < -12.0)||(exponent > 12.0))
	{
	sprintf(buf,"%G",aReal);
	}
else
	{
	if (exponent >= floor(exponent)) exponent = floor(exponent+1);
	sprintf(buf,"%12.12f",aReal);
	/* Do not allow more than 17 dogits of precision */
	bufLen = strlen(buf);
	for (i = 0; i < bufLen; ++i)
	{
		if ((buf[i] >= '0') && (buf[i] <= '9')) ++precision;
...
else //if (wholeNumberSW == FALSE)
	{
	// Remove any trailing zeros IF there is a fraction
	for (i=strlen(buf)-1;(i>1)&&(buf[i]=='0')&&(buf[i-1]!='.');--i) 
		{
		buf[i] = 0;
		}
	}
...
_TObject_RecordFrame
TObject_MarkTval(gCP,gTP,gTP->DebugTraceLambda);
...
if( asTag(&oldValue) == TYQUOTEDPAIR)
    ...
    gCP->TObject_MaxOutputLen = sizeof(gTP->TempBuffer) - 1;
    size = 0;
    gTP->TempBuffer[0] = 0;
	    *retTval = TPair_PrintQuoted(gCP,gTP,oldValue, &size, gTP->TempBuffer);
	gCP->TObject_MaxOutputLen = savsize;
    if(asTag(retTval) == TYBOLE && asBool(retTval) == TRUE)
	    *retTval = TObject_CnvFromText(gCP, gTP, gTP->TempBuffer);
	FrameExit(*retTval);
    }
else
if( asTag(&oldValue) == TYQUOTEDSYMBOL)
    ...
    size = 0;
    gTP->TempBuffer[0] = 0;
    *retTval = TSymbol_PrintQuoted(gCP, gTP, oldValue, &size, gTP->TempBuffer);
	gCP->TObject_MaxOutputLen = savsize;
    if(asTag(retTval) == TYBOLE && asBool(retTval) == TRUE)
		*retTval = TObject_CnvFromText(gCP, gTP, gTP->TempBuffer);
		FrameExit(*retTval);
...
if( asTag(&oldValue) == TYSYMBOL)
    ...
    if (asSymbol(&oldValue)->itNeedsBars == TRUE)
        sprintf((char*)gTP->TempBuffer,"|%s|",(char*)*(asSymbol(&oldValue)->itsCString));
    else
        strcpy((char*)gTP->TempBuffer,(const char*)*(asSymbol(&oldValue)->itsCString));
    *retTval = TObject_CnvFromText(gCP, gTP, gTP->TempBuffer);
    FrameExit(*retTval);
    }
else
if (asTag(&oldValue) == TYCPX)
	...
	size = 0;
    gTP->TempBuffer[0] = 0;
    TCpx_Print(gCP,gTP, oldValue, &size, gTP->TempBuffer);
    *retTval = TObject_CnvFromText(gCP, gTP, gTP->TempBuffer);
    FrameExit(*retTval);
    }
else
if (asTag(&oldValue) == TYSTRINGSUBSTR)
    ...
    if (aStringPtr == NULL)
        ...
        sprintf((char *)gTP->TempBuffer,"#<%s "INTFORMAT">",(const char *)typeName,asObject(&oldValue)->itsObjectIndex);
        aStringPtr = gTP->TempBuffer;
...
if (_TObject_TypeFlag(asTag(&oldValue)) == _TObject_TfTOBJECT)
    ...
    if(strlen((char *)aStringPtr) < sizeof(gTP->TempBuffer) - 1)
        strcpy((char*)gTP->TempBuffer,(const char *)aStringPtr);
    else
        {
        /*  error ... */
...
if ((asTag(&oldValue) == TYNUM) || (asTag(&oldValue) == TYPOINTER))
    {
    sprintf((char *)gTP->TempBuffer,INTFORMAT,asInt(&oldValue));
    *retTval = TObject_CnvFromText(gCP, gTP, gTP->TempBuffer);
    FrameExit(*retTval);
    }
else
if (asTag(&oldValue) == TYUNUM)
    {
    sprintf((char *)gTP->TempBuffer,UINTFORMAT,asUInt(&oldValue));
    *retTval = TObject_CnvFromText(gCP, gTP, gTP->TempBuffer);
    FrameExit(*retTval);
    }
else
if ((asTag(&oldValue) == TYCOMPARE) || (asTag(&oldValue) == TYTYPE))
    {
    sprintf((char *)gTP->TempBuffer,SHORTFORMAT,asShort(&oldValue));
    *retTval = TObject_CnvFromText(gCP, gTP, gTP->TempBuffer);
    FrameExit(*retTval);
    }
else
if (asTag(&oldValue) == TYREAL)
    {
    TObject_sprintReal(gCP,gTP,(char *)gTP->TempBuffer,asReal(&oldValue));
    *retTval = TObject_CnvFromText(gCP, gTP, gTP->TempBuffer);
    FrameExit(*retTval);
    }
else
if (asTag(&oldValue) == TYMONEY)
    {
    gTP->TempBuffer[0] = '$';
    TObject_sprintReal(gCP,gTP,(char *)&gTP->TempBuffer[1],asReal(&oldValue));
    *retTval = TObject_CnvFromText(gCP,gTP,gTP->TempBuffer);
    FrameExit(*retTval);
    }
else
if (asTag(&oldValue) == TYDATE)
    {
	TObject_CnvDateToText(gCP, gTP, gTP->TempBuffer, oldValue);
...
if (asTag(&oldValue) < gCP->TObject_MaxTypes)
    {
    sprintf((char*)gTP->TempBuffer,"#<%s>", _TObject_TypeName(asTag(&oldValue)));
    *retTval = TObject_CnvFromText(gCP,gTP,gTP->TempBuffer);
    FrameExit(*retTval);
    }
else
