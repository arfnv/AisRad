...
*ec = FProcedure_NewCProcedure(gCP,gTP,&aSymbol,(LpCHAR)"getTickCount",(LpFUNC)&FConio_GetTickCount);
ExitOnError(*ec);
*ec = FProcedure_NewCProcedure(gCP,gTP,&aSymbol,(LpCHAR)"getRecursionCount",(LpFUNC)&FConio_GetRecursionCount);
ExitOnError(*ec);
...
ExitOnError(*ec);
*ec = FProcedure_NewCProcedure(gCP,gTP,&aSymbol,(LpCHAR)"writelg",(LpFUNC)&FConio_Writelg);
ExitOnError(*ec);
*ec = FProcedure_NewCProcedure(gCP,gTP,&aSymbol,(LpCHAR)"logConsole",(LpFUNC)&FConio_logConsole);
ExitOnError(*ec);
...
ExitOnError(*ec);
*ec = FProcedure_NewCProcedure(gCP,gTP,&aSymbol,(LpCHAR)"importCsv",(LpFUNC)&FConio_ImportCSV);
ExitOnError(*ec);
*ec = FProcedure_NewCProcedure(gCP,gTP,&aSymbol,(LpCHAR)"exportTab",(LpFUNC)&FConio_ExportTAB);
ExitOnError(*ec);
*ec = FProcedure_NewCProcedure(gCP,gTP,&aSymbol,(LpCHAR)"exportCsv",(LpFUNC)&FConio_ExportCSV);
ExitOnError(*ec);

*ec = FProcedure_NewCProcedure(gCP,gTP,&aSymbol,(LpCHAR)"checkEscape",(LpFUNC)&FConio_CheckEscape);
ExitOnError(*ec);
*ec = FProcedure_NewCProcedure(gCP,gTP,&aSymbol,(LpCHAR)"system",(LpFUNC)&FConio_System);
ExitOnError(*ec);
...
ExitOnError(*ec);
*ec = FProcedure_NewCProcedure(gCP,gTP,&aSymbol,(LpCHAR)"fileDir",(LpFUNC)&FConio_fdir);
ExitOnError(*ec);
...
ExitOnError(*ec);
*ec = FProcedure_NewCProcedure(gCP,gTP,&aSymbol,(LpCHAR)"fileEraseDir",(LpFUNC)&FConio_feraseDir);
ExitOnError(*ec);
*ec = FProcedure_NewCProcedure(gCP,gTP,&aSymbol,(LpCHAR)"fileExists",(LpFUNC)&FConio_fexists);
ExitOnError(*ec);
*ec = FProcedure_NewCProcedure(gCP,gTP,&aSymbol,(LpCHAR)"fileMakeDir",(LpFUNC)&FConio_fmkDir);
ExitOnError(*ec);
...
ExitOnError(*ec);
*ec = FProcedure_NewCProcedure(gCP,gTP,&aSymbol,(LpCHAR)"createBuffer",(LpFUNC)&FConio_CreateBuffer);
ExitOnError(*ec);
*ec = FProcedure_NewCProcedure(gCP,gTP,&aSymbol,(LpCHAR)"deleteBuffer",(LpFUNC)&FConio_DeleteBuffer);
ExitOnError(*ec);
...
Check the context structure for an escape or quit request, and
return 1 if either an escape or a quit has been requested.

#endif

NUM FConio_Escape(LpXCONTEXT gCP, LpTHREAD gTP)
{
NUM		escapeSW = 0;

if (gTP->escapeSW != 0) escapeSW = 1;
...
if (gTP->FConio_LogConsoleSW == TRUE) FConio_IO_writeLog(gCP,gTP,(LpCHAR)&buf[0],0); 

FrameExit(*displayString);
}
...
if (argc > 1)
{
	parmv[0].Tag = TYTEXT;
	parmv[0].u.Text[0] = 0;
	for (indexOf = 0; indexOf < argc; ++indexOf) parmv[indexOf+1] = argv[indexOf];

	*displayString = FUtil2_Append(gCP, gTP, argc+1, parmv);
	ExitOnError(*displayString);
} 
else
if (argc == 1)
...
if (gTP->FConio_LogConsoleSW == TRUE) FConio_IO_writeLog(gCP,gTP,(LpCHAR)&buf[0],1); 
...
The FConio_Writelg function displays the arguments on the AIS log file,
and terminates the display with a new line.

#endif

TVAL FConio_Writelg(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])      
{
NUM                 indexOf;
CHAR*               buf;
TVAL				parmv[50];
...
/* Display the display string in the AIS Log file */
FConio_IO_writeLog(gCP,gTP,(LpCHAR)&buf[0],1);
...
The FConio_logConsole sets the switch which tells the Smartbase engine
to log allFConio_logConsole console writes (writeln display) to the AIS log file.

#endif

TVAL FConio_logConsole(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])      
{
...
/* There must be one boolean argument */
if ((argc == 1) && (argv[0].Tag == TYBOLE))
{
	gTP->FConio_LogConsoleSW = argv[0].u.Bool;
} 
else
{
	return(TERROR("!logConsole: expecting 1 boolean argument"));
}
...
if (aThing.Tag != TYVOID)
	...
	*ret = FConio_sprint(gCP,gTP,_FSmartbase_MAXBUFFERLEN,buf,aThing);
	...
	(*gCP->_Host_Display)((POINTER)gCP,gTP,"", 1);
...
Arguments:
    ...
    fileID  :   NUM file identifier (-1 indicates to close all open files).
...
TVAL FConio_fclose(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[])
{
	...
	intResult = (*gCP->_Host_Closef)((POINTER)gCP,gTP, fileID, fileOption);
...
The FConio_GetColumnTAB scans the current record for a column terminator:Tab or Null byte
...
void FConio_GetColumnTAB(HMChar bufferHdl, register LpNUM recordSize, register LpNUM curCharIndex)
{
...
The FConio_GetColumnCSV scans the current record for a column terminator:Comma or Null byte
(The Null Byte indicates that the column being parsed is the last column of a record)
The curCharIndex pointer is advanced to the character following the Comma or Null.

#endif

void FConio_GetColumnCSV(HMChar bufferHdl, register LpNUM recordSize, register LpNUM curCharIndex)
{
while (*recordSize > 0 && (atHMChar(bufferHdl,*curCharIndex) != ',') && (atHMChar(bufferHdl,*curCharIndex) != 0)) 
	{
	++(*curCharIndex);	/* update the current character index */
	--(*recordSize);	/* reduce the number of bytes to scan in this record */
	}
...
if (*recordSize > 0	&& (atHMChar(bufferHdl,*curCharIndex) == ','))
	{
	atHMChar(bufferHdl,*curCharIndex) = 0;	/* replace the tab character with a null to create a K&R string */
	++(*curCharIndex);					/* move the current character index past the tab */
	--(*recordSize);					/* reduce the number of bytes to scan in this record */
...
The FConio_fdir function returns a String object containing the names of the files and the
sub-directories in the the specified path name (including wild cards). The file and 
sub-directorynames are all separated by #newline characters. 

If the optional attributeSW is present and TRUE, then operating system dependent file/directory
attribute information is appended to the file/directory names.

(setq dirString (fileDir "path_info" attributeSW))

Note:	The path must end in wild card characters such as...
 			path\*			Will return all files and directories
			path\*.		Will return only directories
			path\*.sl		Will return only files with the .sl suffix.

Programmer Note:   This implementation is platform dependent.
#endif

TVAL FConio_fdir(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
HMChar              theData = NULL;
LpCHAR				pathPtr = NULL;
CHAR*               buf = NULL;
CHAR				asciiNumber[256];
NUM					oldLen = 0;
NUM					outLen = 0;
BOLE				attributeSW = FALSE;
...
/* Is there an optional attributeSW argument? */
if ((argc >= 1) && (argv[1].Tag == TYBOLE)) attributeSW = argv[1].u.Bool;	

/*  Get a pointer to the path (with wild cards) name */
if (argc < 1) goto InvalidPathArgument;
switch (argv[0].Tag) 
	{
	case TYTEXT: 
		pathPtr = argv[0].u.Text; 
		break;
	case TYSTRING:
		pathPtr = CharArray(argv[0]); 
		break;
	default:
	InvalidPathArgument:
	*ret = TERROR("!fileDir: mandatory 1st argument must be a path string!");
	goto Last;
	}

/*  Allocate a large buffer to receive the directory listing. */
ret->Tag = TYVOID;
if (theData == NULL)
	{ 
...
//  Note: Three characters are for the "\*" plus NULL appended below.
//        Prepare string for use with FindFile functions.
//		  First, copy the string to a buffer, then append '\*' to the directory name.
StringCchLength(pathPtr,MAX_PATH,&length_of_path);
if (length_of_path > (MAX_PATH - 3))
   {
	*ret = TERROR("!fileDir: path argument exceeded the maximum MSWindows allowed length!");
	goto Last;
   }

// Copy the path string for use with FindFile functions.
// Note: the path must end in wild card characters such as...
//	     path\*			Will return all files and directories
//       path\*.		Will return only directories
//       path\*.sl		Will return only files with the .sl suffix.
StringCchCopy(szDir, MAX_PATH, pathPtr);
...
do
   ...
   if (ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
      {
      // _tprintf(TEXT("  %s   <DIR>\n"), ffd.cFileName);
	  strcpy(&buf[outLen],ffd.cFileName);outLen += strlen(ffd.cFileName);
	  strcpy(&buf[outLen],"<DIR>");outLen += 5;
	  if (attributeSW==TRUE)
		{
		FileTimeToSystemTime(&ffd.ftCreationTime,&systemTime);
		sprintf(&buf[outLen],";CreationTime[%i,%i,%i,%i,%i,%i,%i]",systemTime.wYear,systemTime.wMonth,systemTime.wDay,systemTime.wHour,systemTime.wMinute,systemTime.wSecond,systemTime.wMilliseconds);outLen += strlen(&buf[outLen]);
		FileTimeToSystemTime(&ffd.ftLastAccessTime,&systemTime);
		sprintf(&buf[outLen],";LastAccessTime[%i,%i,%i,%i,%i,%i,%i]",systemTime.wYear,systemTime.wMonth,systemTime.wDay,systemTime.wHour,systemTime.wMinute,systemTime.wSecond,systemTime.wMilliseconds);outLen += strlen(&buf[outLen]);
		FileTimeToSystemTime(&ffd.ftLastWriteTime,&systemTime);
		sprintf(&buf[outLen],";LastWriteTime[%i,%i,%i,%i,%i,%i,%i]",systemTime.wYear,systemTime.wMonth,systemTime.wDay,systemTime.wHour,systemTime.wMinute,systemTime.wSecond,systemTime.wMilliseconds);outLen += strlen(&buf[outLen]);
		sprintf(&buf[outLen],";FileSize[%d]",(4294967296*ffd.nFileSizeHigh)+ffd.nFileSizeLow);outLen += strlen(&buf[outLen]);
		}
	  strcpy(&buf[outLen],"\n");outLen += 1;
	  buf[outLen] = 0;
      }
   ...
   if (outLen >= (_FSmartbase_MAXBUFFERLEN-100))
	   {
		*ret = TERROR("!fileDir: directory String too large!");
		goto Last;
	   }
   }
...
if (dwError != ERROR_NO_MORE_FILES) 
   {
	*ret = TERROR("!fileDir: trailing IO error!");
	goto Last;
   }
...
/* Convert the directory list buffer into a String object */
*ret = TSTRING(buf);

/* Return to caller and release buffered memory */
Last:
if (theData != NULL)
    FMemory_Free(gCP, gTP, (HMemory)theData);
...
The FConio_feraseDir function erases the specified directory. On some operating systems, such as
MS Windows, the directory must be empty before it can be erased.

(fileEraseDir "path_info"))

Programmer Note:   This implementation is platform dependent.
#endif

TVAL FConio_feraseDir(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
LpCHAR				pathPtr = NULL;
StartFrame
DeclareTVAL(ec);
...
/*  Get a pointer to the directory path name */
if (argc < 1) goto InvalidPathArgument;
...
switch (argv[0].Tag) 
	...
	default:
	InvalidPathArgument:
	*ret = TERROR("!fileEraseDir: mandatory 1st argument must be a directory path string!");
	goto Last;
	}
...
//  Erase the specified directory.
ret->u.Bool = RemoveDirectory(pathPtr);
...
The FConio_fmkDir function creates the specified directory. 

(fileMakeDir "path_info"))

Programmer Note:   This implementation is platform dependent.
#endif

TVAL FConio_fmkDir(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
LpCHAR				pathPtr = NULL;
StartFrame
DeclareTVAL(ec);
...
switch (argv[0].Tag) 
	...
	default:
	InvalidPathArgument:
	*ret = TERROR("!fileMakeDir: mandatory 1st argument must be a directory path string!");
	goto Last;
	}
...
//  Erase the specified directory.
ret->u.Bool = CreateDirectory(pathPtr,NULL);
...
The FConio_fexists function returns TRUE if the specified file OR directory exists, and 
returns FALSE otherwise. 

(setq exists (fileExists "path_file_info"))

Note:	The path may end in wild card characters such as...
 			path			Will return true if the file or directory iexists
 			path\*			Will return true if the directory is NOT empty
			path\*.sl		Will return true if ANY files with the specified suffix exist
			path\*.			Will return true if the directory includes any subdirectories.



Programmer Note:	This function calls FConio_fdir to determine if the file OR directory exists.
#endif

TVAL FConio_fexists(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
StartFrame
DeclareTVAL(ec);
DeclareTVAL(ret);
...
*ec = FConio_fdir(gCP,gTP,argc,argv);
ret->Tag = TYBOLE;
if ((ec->Tag==TYERROR)||(ec->Tag==TYVOID)||((ec->Tag==TYTEXT)&&(ec->u.Text[0]==0)))
	ret->u.Bool = FALSE;
else
	ret->u.Bool = TRUE;
...
The system Function sends the String command to the host POSIX shell
program for execution.

An example follows.

    (system "dir *.*")       =>  Tells Windows to issue a Directory listing

#endif

TVAL FConio_System(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])        
{
	char*       apCptr;
	TVAL        aCode;
...
The getTickCount cProcedure returns the number of clock ticks since the startTicks
argument. If the startTicks argument is 0, then the number of ticks since the last
system time reset are returned.

An example follows.

    (getTickCount startTicks)       =>  clock ticks since startTime

#endif

TVAL FConio_GetTickCount(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])        
{
// The clock() function calls behave differently between Windows and Linux. 
// We will use gettimeofday to determine the elapsed time instead for linux in both 32 and 64 bit.
#if defined(_LINUX)
	REAL	aOldTime;
	REAL	aNewTime;
	struct timeval aTv;
...
/*--------------------------------------------------------------------------------------- */
#if 0
FConio_GetRecursionCount

The getRecursionCount cProcedure returns the recursion count of this called function.

Note: This will be at least one recursion level deeper than the caller.

An example follows.

    (getRecursionCount)       =>  current recursion count of the called function

#endif

TVAL FConio_GetRecursionCount(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])        
{
	return(TINT(gTP->RecursionCount));
}
...
fileID             A numeric ID assigned to the file by a fileOpen.
theTarget          The Lambda or class Structure to handle the import.		    
...
if (theTarget->Tag == TYSTRUCTURE)          
{                                          
	ps = (TStructure*)asObject(theTarget);
}                                         
else
{
	*ret = TERROR("!ImportTab: Expecting an Lambda or class Structure for argument 2!");
    goto BadCleanUp;
}
    ...
    if (isRecordsOnly)
		...
		if (theTarget->Tag == TYLAMBDA)
			...
			*recordVector = FSmartbase_Evalv(gCP, gTP, *result, 2, &prmv[0]);
			...
			{
			/* Call the setImport method of the class Structure to store the imported row text. */
			*result = TStructure_GetIV1(gCP, gTP, TOBJ(ps->itsMethods), TSYMBOL("setImport"));
			ExitOnError(*result);
			prmv[0] = *theTarget;
			prmv[1] = *row;
			prmv[2] = FSmartbase_CnvFromText(gCP, gTP, &atHMChar(bufferHdl, curCharIndex));
			*recordVector = FSmartbase_Evalv(gCP, gTP, *result, 3, &prmv[0]);
			ExitOnError(*recordVector);
			curCharIndex += recordSize;
			recordSize = 0;
			}
    ...
    if (isRecordVectors)
		...
		if (theTarget->Tag == TYLAMBDA)
			...
			*recordVector = FSmartbase_Evalv(gCP, gTP, *result, 1, &prmv[0]);
			...
			{
			/* Call the refImport method of the class Structure to get the new row vector. */
			*result = TStructure_GetIV1(gCP, gTP, TOBJ(ps->itsMethods), TSYMBOL("refImport"));
			ExitOnError(*result);
			prmv[0] = *theTarget;
			prmv[1] = *row;
			*recordVector = FSmartbase_Evalv(gCP, gTP, *result, 2, &prmv[0]);
		...
		while (recordSize > 0)
			{
			FConio_GetColumnTAB(bufferHdl, &recordSize, &curCharIndex);
				...
				case COLUMNREC:
					...
					if (theTarget->Tag == TYLAMBDA)
						...
						*result = FSmartbase_Evalv(gCP, gTP, *result, 2, &prmv[0]);
						...
						{
						/* Add a new column, with this name, to the Lambda. */
						*result = TStructure_GetIV1(gCP, gTP, TOBJ(ps->itsMethods), TSYMBOL("setColName"));
						ExitOnError(*result);
						prmv[0] = *theTarget;
						prmv[1] = *col;
						prmv[2].u.Object = (TObject*)aSymbol;
						prmv[2].Tag = aSymbol->itsObjectType;
						*result = FSmartbase_Evalv(gCP, gTP, *result, 3, &prmv[0]);
						ExitOnError(*result);
						}
...
Check to see if an escape request has been made and throw if so.

#endif

TVAL FConio_CheckEscape(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
StartFrame
DeclareTVAL(ret);
EndFrame
...
The FConio_ImportCSV function requires the following arguments:
...
Note:   The importCsv function only handles text files where each record is separated by an eol 
		character.
#endif

TVAL FConio_ImportCSV(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
NUM                 bufferSize = _FSmartbase_MAXIMPORTBUFFERLEN;
BOLE                isSBFfile = TRUE;
BOLE                isRecordsOnly = FALSE;
...
if (argc != 2 || !isNumIndex(&argv[0]))
	{
	*ret = TERROR("!ImportCsv: Expecting a valid fileid for argument 1!");
	FrameExit(*ret);
	}
...
{                                         
	...
	{	
	*ret = TERROR("!ImportCsv: Expecting an Lambda for argument 2!");
    goto BadCleanUp;
}
...
/* The import function will read the .CSV file records into Spreadsheet cells. */
dataSize = 0;
maxColN = 0;
recordCount = 0;
...
while (TRUE)
    ...
    if (isRecordVectors)
		...
		while (recordSize > 0)
			{
			FConio_GetColumnCSV(bufferHdl, &recordSize, &curCharIndex);
			/* curCharIndex points to the byte after the column delimiter */
			/* The number of bytes in the column will be deducted from recordSize */
			switch (recordType)
				{
...
while (continueExporting)
   ...
   /* If we are exporting records only, get the record string here. */
    if (isRecordsOnly)
		...
		{ 
			/* Call the setImport method of the class Structure to store the imported row text. */
			*result = TStructure_GetIV1(gCP, gTP, TOBJ(ps->itsMethods), TSYMBOL("refExport"));
			ExitOnError(*result);
			prmv[0] = *theTarget;
			prmv[1] = TINT(rowIndex);
			*recordsOnly = FSmartbase_Evalv(gCP, gTP, *result, 2, &prmv[0]);
			ExitOnError(*recordsOnly);
		}
    ...
    intResult = (*gCP->_Host_Writef)((POINTER)gCP,gTP, asNumIndex(&argv[0]), strlen((char *)&bufferPtr[0]), (char *)&bufferPtr[0]);
	if (intResult < 0)
		FrameExit(TERROR("!exportTAB: file write IO error!"));
...
(gCP,gTP,exportCsv file Lambda mode)
...
TVAL FConio_ExportCSV(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
NUM                 bufferSize = _FSmartbase_MAXBUFFERLEN;
LpCHAR              bufferPtr;
HMChar              bufferHdl;
...
while (continueExporting)
   ...
   /* If we are exporting records only, get the record string here. */
    if (isRecordsOnly)
		...
		{ 
			/* Call the refExport method of the class Structure to store the imported row text. */
			*result = TStructure_GetIV1(gCP, gTP, TOBJ(ps->itsMethods), TSYMBOL("refExport"));
			ExitOnError(*result);
			prmv[0] = *theTarget;
			prmv[1] = TINT(rowIndex);
		...
		*result = FSmartbase_Evalv(gCP,gTP,*lengthCmd,1,recordVector);
		...
		while (colIndex < maxCols)
			...
			*result = FSmartbase_Refv(gCP,gTP,2,&prmv[0]);
			...
			/*  Append a comma to the end of each column data	*/
			bufferPtr[dataSize - 1] = ',';
			bufferPtr[dataSize] = 0;
			++colIndex;
			} /* End while loop to process row & column data */
    ...
    intResult = (*gCP->_Host_Writef)((POINTER)gCP,gTP, asNumIndex(&argv[0]), strlen((char *)&bufferPtr[0]), (char *)&bufferPtr[0]);
	if (intResult < 0)
		FrameExit(TERROR("!exportCSV: file write IO error!"));
...
fileID		The fileID returned from a previous FConio_IO_fopen invocation ( -1 indicates to close all open files).
opcode	    The file close option. A value of: (0) = erase file after closing, (1) = don't erase file after closing).
...
/* A fileID of -1 indicates that ALL open files should be closed */
if (fileID < 0)
{
	N = MAXFILEHANDLES;
	for (n = 0; n < N; ++n)
...
if (fileHDL != NULL)
{
	result = fclose(fileHDL);
	if ((result>=0) && (opcode == 0)) remove(&gFileIONames[fileID][0]);
}
...
string		The ascii string to be written to STDOUT.
newline	    If 1 then a newline character is to be appended, otherwise no newline character is appended. 

#endif

NUM	 FConio_IO_printf(LpXCONTEXT gCP, LpTHREAD gTP, char*  string,NUM  newline)
{
NUM			result = 0;

/* Issue the write to STDOUT */
result = printf("%s", string);
if (result != 1) return(1);

/* If newline is present, then write a newline to STDOUT */
if (newline == 1) result = printf("%s", "\n");
if (result != 1) return(1);
...
if ((fileID<=0) || (fileID>=MAXFILEHANDLES) || (gFileIOHandles[fileID] == 0))
	{
	return(-1.0); /* Invalid fileID argument */
	}
...
string		The ascii string to be written to the AIS log file.
newline	    If 1 then a newline character is to be appended, otherwise no newline character is appended. 

#endif

NUM	 FConio_IO_writeLog(LpXCONTEXT gCP,LpTHREAD gTP,char*  string,NUM  newline)
{
static BOLE		firstTimeSW = TRUE;
NUM				maxLogFileSize = 100000000;
NUM				strLEN;
NUM				result = 0;
FILE*			fileHDL;
NUM				logFileSize;
...
char*			fileNamePtr;
char			eol[20] = "\n";
TVAL			sym__log = TSYMBOL("_log");
fpos_t			new_pos;
int				whence = SEEK_END;

// Get a pointer to the log file name
if (sym__log.u.Symbol->itsGlobalValue.Tag == TYTEXT)
	fileNamePtr = sym__log.u.Symbol->itsGlobalValue.u.Text;
else
if (sym__log.u.Symbol->itsGlobalValue.Tag == TYSTRING)
	fileNamePtr = CharArray(sym__log.u.Symbol->itsGlobalValue);
else
if (sym__log.u.Symbol->itsGlobalValue.Tag == TYSYMBOL)
	fileNamePtr = SymbolArray(sym__log.u.Symbol->itsGlobalValue);
else
	return(errFileWrite);


// Open the AIS log file before writing.
// Note: If this is the first write, the AIS log file is cleared.
if (firstTimeSW == TRUE)
{
	// Open the log file and clear all existing contents.
	firstTimeSW = FALSE;
	if ((fileHDL = fopen(fileNamePtr,"wb+")) == NULL) return(errFileWrite);
...
else
{
	...
	if ((fileHDL = (NUM)fopen(fileNamePtr,"ab+")) == 0) return(errFileWrite);
}
...
if (strLEN > 0) 
	{
	result = fwrite(string,strLEN,1,fileHDL);
	if (result != 1) 
		{
		if (fileHDL!=NULL) result = fclose(fileHDL);
		FSmartbase_Throw(gCP,gTP,(const NUM)errFileWrite);
...
// Issue the posix file write for new line.
if (newline == 1) 
	{
	result = fwrite(eol,strlen(eol),1,fileHDL);
	if (result != 1) 
		{
		if (fileHDL!=NULL) result = fclose(fileHDL);
		FSmartbase_Throw(gCP,gTP,(const NUM)errFileWrite);
...
// Get the size of the AIS log file.
fseek(fileHDL,0,SEEK_END);
fgetpos(fileHDL,&new_pos);
#ifdef _LINUX
logFileSize = new_pos.__pos;
#else
logFileSize = new_pos;
#endif

// ********************************************************************
// Check the size of the AIS log file to make sure it is not too large.
// ********************************************************************
if (logFileSize >= maxLogFileSize)
	{
	// Read the rightmost bytes of the AIS log file
	newLogFileSize = (maxLogFileSize / 2);
	fseek(fileHDL,logFileSize-newLogFileSize,SEEK_SET);
	result = (NUM)fread(gTP->TempBuffer,newLogFileSize,1,fileHDL);
	if (result < 0)
		{
		if (fileHDL!=NULL) result = fclose(fileHDL);
		FSmartbase_Throw(gCP,gTP,(const NUM)errFileWrite);
	...
	// Close file and reopen as a new file destroying the old contents
	if (fileHDL!=NULL) result = fclose(fileHDL);
	if ((fileHDL = fopen(fileNamePtr,"wb+")) == NULL) return(errFileWrite);

	// Write the rightmost bytes of the AIS log file back to the beginning of the log file
	result = fwrite(gTP->TempBuffer,newLogFileSize,1,fileHDL);
	if (result != 1) 
		{
		if (fileHDL!=NULL) result = fclose(fileHDL);
		FSmartbase_Throw(gCP,gTP,(const NUM)errFileWrite);
...
// Close log file and return no error code
if (fileHDL!=NULL) result = fclose(fileHDL);
return(0);
}
...
Create a buffer space external to the Smartbase heap.

(setq bufferPtr (createBuffer bufferSize))



#endif
TVAL FConio_CreateBuffer(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{ 
	POINTER		apBlock;
	StartFrame
	DeclareTVAL(aSize);
...
Deletes a buffer space external to the Smartbase heap.

(deleteBuffer (setq bufferPtr (createBuffer bufferSize)))



#endif
TVAL FConio_DeleteBuffer(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{ 
	StartFrame
	DeclareTVAL(aRet);
	EndFrame
