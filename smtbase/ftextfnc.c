...
{
NUM			maxPostDecimalPrecision = 30;	
CHAR        sprintFormat[20];
CHAR        xlFmt[128];
CHAR        prefix[8];
CHAR        postfix[8];
CHAR        preDecimal[512];
CHAR        postDecimal[512];
CHAR        postDecimalFormat[512];
LpCHAR      tmpP;
REAL        tmpReal;
REAL        floorValue;
REAL        sign;
...
prefix[0] = postfix[0] = preDecimal[0] = postDecimal[0] = postDecimalFormat[0] = 0;
...
if ((tmpP = (LpCHAR)strchr((char*)xlFmt, '.' )) != NULL)
    ...
    while(*tmpP && (*tmpP == '#' || *tmpP == '0'))
        {
		postDecimalFormat[places] = *tmpP;
        tmpP++;
        places++;
  		postDecimalFormat[places] = 0;
       }
	if (places > maxPostDecimalPrecision)
		{
			FrameExit(TERROR("!text: post decimal real number format (.###) precision must be 30 digits or less!"));
		}
    ...
    sprintFormat[0] = '%';
    sprintFormat[1] = '.';
    sprintf((char*)&sprintFormat[2], "%if", places); // Final format will look something like "%.20f" where 20 is the contexts of places
    sprintf((char*)&buff[0], sprintFormat, tmpReal);
    strncpy((char*)&postDecimal[0], (char*)&buff[1], places+1);
    postDecimal[places+1] = 0;
    
    // Now drop all trailing '0' chars from the rightmost end of the fraction IFF the format char is '#'.
	// Note: postDecimal[n] is offset 1 position from postDecimalFormat[n] because of the decimal '.' point.
    n = places-1;
	for (n = places;((n >= 0)&&((postDecimal[n] == '0') && (postDecimalFormat[n-1] == '#')));--n) postDecimal[n] = 0;
    }
else
    {
    postDecimal[0] = 0;
