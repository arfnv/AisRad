...
gCP->FVmscript_Symbol_debugDetective		= TSymbol_MakeUnique(gCP,gTP,(LpCHAR)"debugDetective");
FObject_Perm(gCP,gTP,(TObject*) gCP->FVmscript_Symbol_debugDetective,TRUE);
...
/*  ===================================================================== */
++proc->InUse;
if ((gTP->DebugJitON == TRUE) && (*JIT != NULL))
    {
...
TVAL                    errorMsg;               /*  Smartbase Error Message (if any) */
...
if (self->ClassVariables == NIL)
	...
	if ((argc >= 1) && (self->ArgumentVariables->itsMaxItemIndex >= 1) && (BindArray(TOBJ(self->ArgumentVariables))[0].Key == (TObject*)gCP->TLambda_self))
		...
		/* Set the Sv register to the contents of the self argument */
		Sv = argv[0].u.Structure;
		}
	else
		{
		// MFK goto SelfArgumentError; 
		}
	}
...
/*        facility is NOT turned on.                                      */ 
/*  ===================================================================== */
if (gTP->DebugTraceON == TRUE)
    {NUM tempCodeL = (Ip - 1 - &atHMInt(Pc->itsInstructionArray,0));
    if (((gTP->DebugTraceLambda.Tag!=TYNUM)||(gTP->DebugTraceLambda.u.Int!=-2)) &&
		(gTP->DebugTraceCodeL != -2) &&
		(gTP->DebugTraceRecursion != -2) &&
		(((gTP->DebugTraceLambda.Tag==TYNUM)&&(gTP->DebugTraceLambda.u.Int==-1)) || (gTP->DebugTraceLambda.u.Lambda == proc)) &&
		((gTP->DebugTraceCodeL == -1) || (gTP->DebugTraceCodeL == (Ip - 1 - &atHMInt(Pc->itsInstructionArray,0)))) &&
		((gTP->DebugTraceRecursion == -1) || (gTP->RecursionCount <= gTP->DebugTraceRecursion))
        )
        {
        /*  ===================================================================== */
        ...
        /*  Construct the input for the debugDective function. */
		gTP->DebugTraceON == FALSE;
		gTP->DebugSuspended == TRUE;
        FVmScript2_DebugManager(gCP,gTP,self,argc,argv,Pc,Pv,Sv,Rp,Vs,VsTopOfStack,Fb,Ip,gCP->Tval_VOID);
				...
				switch (index->Tag)
					...
					case TYQUOTEDSYMBOL:
						if ((inRange(asMemo(index),0,Structure(isource)->itsMaxItemIndex)) &&
							...
							if (gTP->FVmscript_Selection != 0)
								{
								/* If the index was not found in the Structure, check to see if the index is a class method. */
								if (isource.u.Structure->itsMethods != NIL)
									{
									retValue = TStructure_GetIV1(gCP, gTP, TOBJ(isource.u.Structure->itsMethods), *index);
									}
								else
									retValue = gCP->Tval_VOID;
								}
	  ...
	  NestedReturn:
		...
		if (retValue.Tag == TYERROR)
			...
			if (strcmp(ErrorArray(retValue),ErrorArray(gCP->TObject_ERROR_CONTINUATION)) != 0)
				...
				retValue = TERROR(gTP->TempBuffer); 
				errorMsg = TSTRING(gTP->TempBuffer); 
				...
				/*  Call debugDetective if we are in instruction trace mode and trace on error is on. */
				if ((gTP->DebugTraceON == TRUE) && (gTP->DebugErrorON == TRUE))
		                FVmScript2_DebugManager(gCP,gTP,self,argc,argv,Pc,Pv,Sv,Rp,Vs,VsTopOfStack,Fb,Ip+1,errorMsg);
				}
			}
