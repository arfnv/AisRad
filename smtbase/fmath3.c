...
ExitOnError(*ec);
*ec = FProcedure_NewCProcedure(gCP,gTP,&aSymbol,(LpCHAR)"sigmoid",(LpFUNC)&FMath3_Sigmoid);
ExitOnError(*ec);
...
Return the sigmoid function of a numeric argument. 

Note:   Exactly one argument is expected, anything else is an error. 
  
#endif

TVAL FMath3_Sigmoid(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc,TVAL argv[])      
{
REAL			x;
StartFrame
DeclareTVAL(ret);
