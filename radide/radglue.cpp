...
1.0000  12/14/2013	mfk	    Initial experimentation.
												--------------- ---------------
NOTES
RADGLUE is the "glue" between the two components of the system. It provides the following:
...
- Callback interface for SmartBase engine callbacks. These functions convert data types etc. and then make calls on the
 RADIDE as necessary. If the function can be fully implemented in this file we do so. The callbacks are functions so
...
- Utility functions. Functions to be used by other functions in this file and by the RADIDE. These functions must only
 use generic C types and can not pass or return structures. If you find this too hard to live with then write a separate
 utility function for the RADIDE and RADGLUE. 

- Local functions. Functions to be used only by functions in this file that were called by the engine. 

Note that only those functions called by the RADIDE should appear in RADGLUE.h. The other functions are meant to be
...
RADIDE. All core functions that must be registered with the engine are found in this file. 

When we can handle all of the required fucntionality in this file we do it. Many callback functions require that we call a companion
function in the RADIDE. There are also functions that are here only to allow depreciated, obsolete or irrelevent calls,
...
#include "radglue.h"		// Functions callable by RADIDE
...
/********************************************* PUBLIC C++ FUNCTIONS **********************************************************
The following functions represent the external interface of the RADGLUE file. These functions are primarily called by the
RADIDE C++ code.  These functions can only use standard C types as the fsmtbase.h file cannot be included in the calling
program (e.g. RADIDE).
******************************************************************************************************************************/

/*-----------------------------------------------------------------
RADGLUE_CalculateStackSpace is called by the host application to 
...
#if 0 // MFK
/* ****************************************************************************************************************
Note: Waiting to be converted and implemented.
******************************************************************************************************************/
enum AClientResultType {geBooleanContextClientResultType,
                        geNumberContextClientResultType,
						geStringContextClientResultType,
	                    geStructureContextClientResultType
                       }; 

QMutex gContextMutex[RADGLUE_MAX_CONTEXTS];
...
REAL gStartClock;
AContextFlags gContextFlags[RADGLUE_MAX_CONTEXTS];
...
// file scope globals holding callback pointers. See RADGLUE_Init().
...
RADGLUE.h  as they usually have references to types defined in fsmtbase.h.
******************************************************************/
...
/*-----------------------------------------------------------------
RADGLUE_FlushLispCodeInContext
...
Return:	retCode		The RADGLUE return code

----------------------------------------------------------------*/
long RADGLUE_FlushLispCodeInContext(void* ipContext)
{
	LpXCONTEXT	gCP;
	LpTHREAD	gTP;
	NUM			aCatchCode;
...
LBLSystemThrowError:
	...
	// as AIS may be shutting down when RADGLUE_FlushLispCodeInContext was called.
...
------------------------------------------------------------------*/
long RADGLUE_Eval(void *ipCP, long iSessionID, long aRequestID, long iUserID, long iLevel,long iEvalType, long iNoDebug,long iSetJmp, long iJitMode, char* ipCmdString, char* ipData)
{
	ASession*	apSession;
	char*		aAmpCmd = NULL;
	LpXCONTEXT	gCP;
...
#define SBRESULTBUFSIZE 4096
	...
	gTP->SessionID = iSessionID; // Set sessionID so that all callbacks from the engine have
								// access to it. Each callback into SessionManager from RADGLUE
	...
	if (apSession == NULL)
		FrameExit(RADGLUE_EVAL_FAILURE);
	...
	// a new request nested inside a previous call to RADGLUE_Eval. For example, we could have
	// called RADGLUE_Eval and then had the engine call RADGLUE_DebugDialog which might then 
	// (depending on the client side implementation RADGLUE_DebugDialog calls) make another
	// RADGLUE_Eval call. Without this check the second call would reset the throw environment 
	// and if the original RADGLUE_Eval processing was interrupted by an escape condition the 
	...
	*aCmdDelete = TGVALUE("delete");
	...
	if (gTP->DebugSuspended == FALSE) {					// The jit mode is normally RADGLUE_JITON (0x10).
	...
	switch(iEvalType)
	{
	case RADGLUE_EVENTCMD:	// provide no return aResult
	case RADGLUE_CMDSTRING:	// provide return aResult
	case RADGLUE_CMDSTRING_BINTRANSFER: // provide return aResult and binary data, see execute.
									 // also handle incomming binary data, see execute
		if (iLevel < SYSGLUESECLEVEL)
		{	gpSessionManager->cbReturnResult(gTP->SessionID, gTP->RequestID, errorMsg(errorNum(RADGLUE_ERR_EVAL_SECURITY))
			, "text/plain", NULL, NULL/*Data*/, 0/*DataSize*/);
			FrameExit(RADGLUE_ERR_EVAL_SECURITY);
		}
		...
		{	
			if (iEvalType == RADGLUE_CMDSTRING_BINTRANSFER && ipData != NULL)
			...
			if (aAmpCmd == 0 && iEvalType == RADGLUE_CMDSTRING_BINTRANSFER && ipData != NULL)
		...
		if (iEvalType == RADGLUE_EVENTCMD) // Skip sending return aResult if this is an event
			FrameExit(RADGLUE_EVAL_SUCCESS);

		if (iEvalType == RADGLUE_CMDSTRING_BINTRANSFER)
		...
		case RADGLUE_CHECKIN:
			if (iLevel < SYSGLUESECLEVEL)
			{	gpSessionManager->cbReturnResult(iSessionID, aRequestID, errorMsg(errorNum(RADGLUE_ERR_EVAL_SECURITY))
				,"text/plain", NULL, NULL/*Data*/, 0/*DataSize*/);
				FrameExit(RADGLUE_ERR_EVAL_SECURITY);
			}
			// Extract Extent Name
			for (aIx=0; aIx < aCmdStringLen && ipCmdString[aIx] != '\177'; aIx++)
				; // find next del
			if (aIx == aCmdStringLen)
			{	gpSessionManager->cbReturnResult(iSessionID, aRequestID, errorMsg(errorNum(RADGLUE_ERR_EVAL_BAD_CHECKIN))
				,"text/plain", NULL, NULL/*Data*/, 0/*DataSize*/);
				FrameExit(RADGLUE_ERR_EVAL_BAD_CHECKIN);
			}
			...
			*aLambdaName = TSTRING(&ipCmdString[aTokenStart]);
			if (aIx == aCmdStringLen)
			{	gpSessionManager->cbReturnResult(gTP->SessionID, gTP->RequestID, errorMsg(errorNum(RADGLUE_ERR_EVAL_BAD_CHECKIN))
				, "text/plain", NULL, NULL/*Data*/, 0/*DataSize*/);
				FrameExit(RADGLUE_ERR_EVAL_BAD_CHECKIN);
			}
		...
		case RADGLUE_MSGBLOCK: // Amp Message
		case RADGLUE_EVENTMSG: // Amp message but no return aResult generated
MESSAGE_BLOCK_ENTRY:
			...
			*aMsgBlockVector = FSmartbase_Eval(gCP,gTP,*aCmdStringToVector,4,*aMsgBlockString,*aRuboutChr,TBOOL(true),TBOOL(true));
			...
			if (aResult->Tag == TYERROR)
				break; // out of RADGLUE_MSGBLOCK case
			...
			{	// Convert message block vector to structure
 				if (iEvalType == RADGLUE_CMDSTRING_BINTRANSFER && aAmpCmd != 0)
				...
				*aResult = FSmartbase_Eval(gCP,gTP,*aLambda,1,*aMsgBlockStructure);
				if (aResult->Tag == TYERROR)
					break;
				if (iEvalType == RADGLUE_EVENTMSG)
					FrameExit(RADGLUE_EVAL_SUCCESS);	// goto SKIP_RETURN_RESULT;
				...
				{	// Process the result structure. Return types are limited when you use
					...
					if (aResult->Tag != TYERROR)
					{
						...
						FrameExit(RADGLUE_EVAL_SUCCESS);	// SKIP_RETURN_RESULT
					}
					else
						FrameExit(RADGLUE_EVAL_FAILURE);
			...
			break; // out of case RADGLUE_MSGBLOCK
		}
	if (aResult->Tag == TYERROR)
	{	
		if (apData != NULL)
	...
	// Should never get here so return a RADGLUE error
	...
	if ((aCatchCode == RADGLUE_ERR_FILE_READ) || (aCatchCode == RADGLUE_ERR_FILE_WRITE))
	{	SysGlue_lastError(gCP,gTP,aErrorStringBuffer);
...
long RADGLUE_Init()
{
	...
	for (long i = 0; i < RADGLUE_MAX_CONTEXTS; ++i)
...
this routine.
------------------------------------------------------------------*/
long RADGLUE_OpenContext(const char * ipContextName,const char* ipScript,/*out*/ void ** ipContext, NUM iMemorySize, long objHdrSize,
...
/* 
RADGLUE_SetEngineFlags changes the state of the requested engine flags.
...
Modified flag to 1.
*/
long RADGLUE_SetEngineFlags(long iContextIndex,long iFlag, long iFlagValue)
{
...
TVAL	RADGLUE_AXml(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_Browse(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_BuildLambdaMessage(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_SetResult(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_ContextClient(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[]);
TVAL	RADGLUE_CpOSFile(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[]);
TVAL	RADGLUE_DebugDialog(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[]);
TVAL	RADGLUE_DecodeURL(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[]);
TVAL	RADGLUE_Dir(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[]);
NUM		RADGLUE_Display(LpXCONTEXT gCP,LpTHREAD gTP, char* ipString, NUM  iNewline);
TVAL	RADGLUE_DisplayWorkbenchWindow(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_EnableConsoleLog(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_EncodeURL(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[]);
NUM		RADGLUE_Escape(LpXCONTEXT gCP, LpTHREAD gTP);
TVAL	RADGLUE_EvalAMP(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_EvalsInSyncLocalContext(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_ExistsOSFile(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[]);
TVAL	RADGLUE_FileInfo(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[]);
TVAL	RADGLUE_GetConsoleLog(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_GetContextPtr(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[]);
TVAL	RADGLUE_GetFileStatus(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_GetHttp(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_GetOSDir(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_GetTickCount(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_HostCreate(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_HostDestroy(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_HostGetProperty(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_HostInvoke(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_HostPutProperty(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_HostSend(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_Input(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_LoadBrowseLambda(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_LoadLib(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_LoadUtilityLambdas(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_MkOSDir(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_MsgBox(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_NextUserID(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_Notify(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
NUM		RADGLUE_Open(LpXCONTEXT gCP, LpTHREAD gTP, char* ipName, NUM iMode, NUM iType);
TVAL	RADGLUE_OpenConsoleLog(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_PostHttp(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_ReadHtmlPage(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_ReloadLib(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_RingBell (LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[]);
TVAL	RADGLUE_Select(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_SendToClient(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_ServerDoomed(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_SmOpenContext(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_Sleep(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_Submit(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[]);
TVAL	RADGLUE_Subscribe(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[]);
TVAL	RADGLUE_System(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[]);
TVAL	RADGLUE_TcpClient(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[]);
TVAL	RADGLUE_Throw(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[]);
TVAL	RADGLUE_OpenLog(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_StartLogging(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_StopLogging(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_CloseLog(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
void	RADGLUE_WriteTime(LpXCONTEXT gCP,LpTHREAD gTP);
NUM		RADGLUE_UpdateState(LpXCONTEXT gCP, LpTHREAD gTP);
TVAL	RADGLUE_Md5Hash(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_AddToFileWatcher(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_RemoveFromFileWatcher(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_SendFileChangedEvent(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_SendStatusUpdateEvent(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_ContextOpen(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_ContextSubmit(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_ContextBusy(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_ContextExists(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_ContextStop(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_ContextGetResult(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_ContextGetDisplay(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_ContextClose(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_ContextMyself(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_ContextSubscribe(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[]);
TVAL	RADGLUE_ContextList(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[]);
...
TVAL RADGLUE_AXml(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	if (argc < 2)
		FrameExit(TERROR((LpCHAR)"RADGLUE_AXml, arglist")); // make sure we have the minimum number of arguments
...
TVAL RADGLUE_Browse(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	return(TERROR("!browse not supported by RADGLUE!"));
}
...
/*-----------------------------------------------------------------
RADGLUE_CreateBuffer
The RADGLUE_CreateBuffer function allocates a buffer of the specified
size and returns a pointer to that buffer.
------------------------------------------------------------------*/
TVAL RADGLUE_CreateBuffer(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{ 
	POINTER		apBlock;
	StartFrame
	DeclareTVAL(aSize);
...
/*-----------------------------------------------------------------
RADGLUE_DeleteBuffer
The RADGLUE_DeleteBuffer function deallocates a buffer.
------------------------------------------------------------------*/
TVAL RADGLUE_DeleteBuffer(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{ 
	StartFrame
	DeclareTVAL(aRet);
	EndFrame
...
TVAL RADGLUE_TcpClient(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[])
{
	...
	if (argc < 2) 
		FrameExit(TERROR((LpCHAR)"RADGLUE_Dir, arglist")); // make sure we have the minimum number of arguments

    apATcpClient = (ATcpClient *)asFunction(&argv[0]); // Get pointer to ATcpClient object if any    
	aFunctionHandle = asInt(&argv[1]); // get aFunctionHandle for use in following dispatch table

	// Note that function arguments are passed in a single vector which is the forth argument of the RADGLUE_ContextClient function.
	aArgsLen = 0; // Default to no arguments
	if (argc == 3)
	{	// Get Lambda arguments from arguments vector.
		*aArgs = argv[2];
		if (aArgs->Tag != TYVECTOR)
			FrameExit(TERROR((LpCHAR)"RADGLUE_TcpClient: bad arguments vector"));
	...
	if (apATcpClient == 0 && aFunctionHandle != 10) 
				FrameExit(TERROR((LpCHAR)"RADGLUE_TcpClient:new, no Tcp Client object"));
	switch(aFunctionHandle)
	{	
	//_CleanUp
	case 0:		//_CleanUp - free resource allocated in the glue layer
	...
	case 10: //new - Allocate C++ ATcpClient object
        if (aArgsLen > 0)
            FrameExit(TERROR((LpCHAR)"RADGLUE_TcpClient:new, arglist"));
	...
	case 20:	//(_AISTcpClient objPtr 20 #)
		if (aArgsLen != 2 || (aArg1->Tag != TYSTRING && aArg1->Tag != TYTEXT) || aArg2->Tag != TYNUM)
			FrameExit(TERROR((LpCHAR)"RADGLUE_TcpClient:openTcpConnection, host address and port number required."));
	...
	case 30:	//(_AISContextClient objPtr 30 #)
        if (aArgsLen > 0)
            FrameExit(TERROR((LpCHAR)"RADGLUE_TcpClient:IsConnected, no arguments required"));
	...
	case 40:	//(_AISTcpClient objPtr 40 #)
		if ((aArgsLen != 2)|| (aArg1->Tag != TYSTRING && aArg1->Tag != TYTEXT)
            ...
            FrameExit(TERROR((LpCHAR)"RADGLUE_TcpClient:submit, URL and Http method required"));	
	...
	case 50:	//(_AISTcpClient objptr 50 #)
		if (aArgsLen > 0)
            FrameExit(TERROR((LpCHAR)"RADGLUE_TcpClient:closeConnection, no arguments required"));
...
TVAL RADGLUE_ContextClient(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[])
{
	...
	{	// Get Lambda arguments from arguments vector.
		*aArgs = argv[2];
		if (aArgs->Tag != TYVECTOR)
			FrameExit(TERROR((LpCHAR)"RADGLUE_ContextClient: bad arguments vector"));
		aArgsLen = FSmartbase_VectorLen(gCP,gTP,*aArgs);
		if (aArgsLen > 0)
		{	*aArg1 = FSmartbase_Ref(gCP,gTP,2,*aArgs,TINT(0));
			if (aArgsLen > 1)
	...
	if (apClient == 0 && aFunctionHandle != 10) 
				FrameExit(TERROR((LpCHAR)"RADGLUE_ContextClient:logon, no ContextClient object"));
	...
	case 10:		//new - Allocate C++ ContextClient object
		if (aArgsLen != 2 || (aArg1->Tag != TYSTRING && aArg1->Tag != TYTEXT) || aArg2->Tag != TYNUM)
			FrameExit(TERROR((LpCHAR)"RADGLUE_ContextClient:new, arglist"));
	...
	case 20:	//(_AISContextClient objPtr 20 #(returnType notifyType))
		...
		|| (aArgsLen > 1 && (aArg2->Tag != TYNUM || !getEvalNotifyType(asInt(aArg2), &aEvalNotifyType))))
			FrameExit(TERROR((LpCHAR)"RADGLUE_ContextClient:openConnection, arglist"));		
	...
	case 30:	//(_AISContextClient objptr 30 #(userName password returnType notifyType))
		...
		if ((aArgsLen < 2 || aArgsLen > 4) || (aArg1->Tag != TYSTRING && aArg1->Tag != TYTEXT)
			...
			FrameExit(TERROR((LpCHAR)"RADGLUE_ContextClient:logon, arglist"));		
	...
	case 40:	//(_AISContextClient objptr 40 #(startupPath contextName returnType notifyType))
		...
		if ((aArgsLen < 2)
			...
			||	(aArgsLen > 2 && (aArg3->Tag != TYNUM || !getEvalReturnType(asInt(aArg3), &aEvalReturnType)))
			||	(aArgsLen > 3 && (aArg4->Tag != TYNUM || !getEvalNotifyType(asInt(aArg4), &aEvalNotifyType))))
			FrameExit(TERROR((LpCHAR)"RADGLUE_ContextClient:openContext, arglist"));		
	...
	case 50:	//(_AISContextClient objPtr 50 #(contextName returnType notifyType))
		if (aArgsLen < 1 
			...
			FrameExit(TERROR((LpCHAR)"RADGLUE_ContextClient:openSession, arglist"));		
	...
	case 60:	//(_AISContextClient objptr 60 #(command returnType notifyType))
		...
		if ((aArgsLen < 1 || aArgsLen > 3)
			...
			||	(aArgsLen > 1 && aArg2->Tag != TYNUM) || !getEvalReturnType(asInt(aArg2), &aEvalReturnType)
			||	(aArgsLen > 2 && aArg3->Tag != TYNUM) || !getEvalNotifyType(asInt(aArg3), &aEvalNotifyType))
			FrameExit(TERROR((LpCHAR)"RADGLUE_ContextClient:eval, arglist"));
	...
	case 70:	//(_AISContextClient objptr 70 #(returnType notifyType))
		...
		if ((aArgsLen > 0 && (aArg1->Tag != TYNUM || !getEvalReturnType(asInt(aArg1), &aEvalReturnType)))
			||	(aArgsLen > 1 && (aArg2->Tag != TYNUM || !getEvalNotifyType(asInt(aArg2), &aEvalNotifyType))))
			FrameExit(TERROR((LpCHAR)"RADGLUE_ContextClient:logoff, arglist"));		
		if (aTrace)
		{	qDebug("******** %s %ld --> %s %s %ld ********",gCP->ContextName, gTP->SessionID, apClient->getServerAddress()
			.toLatin1().data(), apClient->getContextName().toLatin1().data(), apClient->getSessionID());
	...
	case 80:	//(_AISContextClient objptr 80 #(contextName returnType notifyType))
		...
		if ((aArgsLen < 1)
			...
			||	(aArgsLen > 1 && (aArg2->Tag != TYNUM || !getEvalReturnType(asInt(aArg2), &aEvalReturnType)))
			||	(aArgsLen > 2 && (aArg3->Tag != TYNUM || !getEvalNotifyType(asInt(aArg3), &aEvalNotifyType))))
			FrameExit(TERROR((LpCHAR)"RADGLUE_ContextClient:closeContext, arglist"));		
		FSmartbase_StringPtr(gCP,gTP,aArg1,&apStr,&aStrlen); // startupPath
		if (aTrace)
		{	qDebug("******** %s %ld --> %s %s %ld ********",gCP->ContextName,gTP->SessionID,apClient->getServerAddress()
			.toLatin1().data(), apClient->getContextName().toLatin1().data(),apClient->getSessionID());
	...
	case 110:	//(_AISContextClient objPtr 110 #(returnType notifyType))
		if (aArgsLen > 2 
			...
			|| (aArgsLen > 1 && (aArg2->Tag != TYNUM || !getEvalNotifyType(asInt(aArg2), &aEvalNotifyType))))
			FrameExit(TERROR((LpCHAR)"RADGLUE_ContextClient:getCurrentContexts, arglist"));		
	...
	FrameExit(TERROR("!function not supported by RADGLUE_ContextClient yet!"));
...
TVAL RADGLUE_DebugDialog(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[])
{
	...
	*aDebugLog = FSmartbase_Eval(gCP, gTP, *aAppendCmd, 10, *aDebugLog, argv[0], *aRuboutChr, argv[1],
	...
	//	RADGLUE_Escape(gCP,gTP); // make sure engine flags get restored properly
	if (aRetcode < 0) 
...
TVAL RADGLUE_Dir(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	// Note that function arguments are passed in a single vector which is the third argument of the RADGLUE_Dir function.
	apArgsLen = 0; // Default to no arguments
	if (argc == 3)
	{	// Get Lambda arguments from arguments vector.
		*aArgs = argv[2];
		if (aArgs->Tag != TYVECTOR)
			FrameExit(TERROR((LpCHAR)"RADGLUE_Dir: bad arguments vector"));
	...
	case 1: //abssoluteFilePath
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:absoluteFilePath, no dir object"));
		if (apArgsLen != 1) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:absoluteFilePath, arglist"));
		...
		*aResult = FSmartbase_CnvFromText(gCP, gTP, (char*)aTmpStr.toLatin1().data());
		...
		break;
	case 2: //absolutePath
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:absolutePath, no dir object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:absolutePath, arglist"));
		aTmpStr = apDirObjectPtr->absolutePath();
		*aResult = FSmartbase_CnvFromText(gCP, gTP,(char*)aTmpStr.toLatin1().data());
		FrameExit(*aResult);
		break;
	case 3: //canonicalPath
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:canonicalPath, no dir object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:canonicalPath, arglist"));
		aTmpStr = apDirObjectPtr->canonicalPath();
		*aResult = FSmartbase_CnvFromText(gCP, gTP,(char*)aTmpStr.toLatin1().data());
		FrameExit(*aResult);
		break;
	case 4: //cd
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:cd, no dir object"));
		if (apArgsLen != 1) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:cd, arglist"));
		...
		*aResult = FSmartbase_CnvFromText(gCP, gTP, (char*)aTmpStr.toLatin1().data());
		...
		break;
	case 5: //cdUp
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:cdUp, no dir object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:cdUp, arglist"));
		aTmpBool = apDirObjectPtr->cdUp();
		*aResult = TBOOL(aTmpBool);
		FrameExit(*aResult);
		break;
	case 6: //cleanDirPath
		FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:cleanDirPath, obsolete"));
		break;
	case 7: //currentDirPath
		FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:currentDirPath, obsolete"));
		break;
	case 8: //convertToAbsolute
		FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:convertToAbs, obsolete"));
		break;
	case 9: //convertSeparators
		if (apArgsLen != 1) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:convertSeparators, arglist"));
		...
		*aResult = FSmartbase_CnvFromText(gCP,gTP,(char*)aTmpStr.toLatin1().data());
		...
		break;
	case 10: //count
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:count, no dir object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:count, arglist"));
		aTmpInt = apDirObjectPtr->count();
		*aResult = TINT(aTmpInt);
		FrameExit(*aResult);
		break;
	case 11: //current
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:current, arglist"));
		aTmpDir = QDir::current();
		apDirObjectPtr = new QDir(aTmpDir.path());
		FrameExit(TFUNCTION(apDirObjectPtr));
		break;
	case 12: //dirName
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:dirName, no dir object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:dirName, arglist"));
		aTmpStr = apDirObjectPtr->dirName();
		*aResult = FSmartbase_CnvFromText(gCP,gTP,(char*)aTmpStr.toLatin1().data());
		FrameExit(*aResult);
		break;
	case 13: //drives
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:drives, arglist"));
	...
	case 14: //entryInfoList
		if (apArgsLen != 3) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:entryInfoList, arglist"));
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:entryInfoList, no dir object"));
		...
		*aTmpVector = FSmartbase_Eval(gCP, gTP, TGVALUE("new"), 2, TSYMBOL("Vector"), TINT(aN));
		...
		break;
	case 15: //entryList
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:entryList, no dir object"));
		if (apArgsLen != 3) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:entryList, arglist"));
		aTmpFilterSpec = (aArg2->Tag == TYVOID) ? apDirObjectPtr->filter() : (QDir::Filters)asInt(aArg2);
		aTmpSortSpec = (aArg3->Tag == TYVOID) ? apDirObjectPtr->sorting() : (QDir::SortFlags)asInt(aArg3);
		FSmartbase_StringPtr(gCP, gTP, aArg1, &apStr, &apStrlen); // nameFilter argument
		...
		*aTmpVector = FSmartbase_Eval(gCP, gTP, TGVALUE("new"), 3, TSYMBOL("Vector"), TSYMBOL("object"), TINT(aN));
		...
		break;
	case 16: //exists
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:exists, no dir object"));
		if (apArgsLen > 1) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:exists, arglist"));
		if (apArgsLen == 1)
		{	FSmartbase_StringPtr(gCP, gTP, aArg1, &apStr, &apStrlen); // fileName argument
		...
		break;
	case 17: //filePath
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:filePath, no dir object"));
		if (apArgsLen != 1) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:filePath, arglist"));
		FSmartbase_StringPtr(gCP, gTP, aArg1, &apStr, &apStrlen); // fileName argument
		...
		*aResult = FSmartbase_CnvFromText(gCP, gTP, (char*)aTmpStr.toLatin1().data());
		...
		break;
	case 18: //filter
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:filter, no dir object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:filter, arglist"));
		aTmpStr = apDirObjectPtr->filter();
		*aResult = FSmartbase_CnvFromText(gCP, gTP, (char*)aTmpStr.toLatin1().data());
		FrameExit(*aResult);
		break;
	case 19: //fileInfoList
		if (apArgsLen != 3) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:fileInfoList, arglist"));
		FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:fileInfoList, not implemented yet"));
		break;
	case 20: //home
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:home, arglist"));
		aTmpDir = QDir::home();
		apDirObjectPtr = new QDir(aTmpDir.path());
		FrameExit(TFUNCTION(apDirObjectPtr));
		break;
	case 21: //homePah
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:homePath, arglist"));
		aTmpStr = QDir::homePath();
		*aResult = FSmartbase_CnvFromText(gCP,gTP,(char*)aTmpStr.toLatin1().data());
		FrameExit(*aResult);
		break;
	case 22: //isReadable
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:isReadable, no dir object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:isReadable, arglist"));
		aTmpBool = apDirObjectPtr->isReadable();
		FrameExit(TBOOL(aTmpBool));
		break;
	case 23: //isRelative
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:isRelative, no dir object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:isRelative, arglist"));
		aTmpBool = apDirObjectPtr->isRelative();
		FrameExit(TBOOL(aTmpBool));
		break;
	case 24: //isRelativePath
		if (apArgsLen != 1) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:isRelativePath, arglist"));
		FSmartbase_StringPtr(gCP,gTP,aArg1,&apStr,&apStrlen); // fileName argument
		aTmpBool = QDir::isRelativePath(apStr);
		FrameExit(TBOOL(aTmpBool));
		break;
	case 25: //isRoot
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:isRoot, no dir object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:isRoot, arglist"));
		aTmpBool = apDirObjectPtr->isRoot();
		FrameExit(TBOOL(aTmpBool));
		break;
	case 26: //match
		if (apArgsLen != 2) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:match, arglist"));
	...
	case 27: //matchAllDirs
		FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:matchAllDirs, obsolete"));
		break;
	case 28: //mkDir
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:mkdir, no dir object"));
		if (apArgsLen != 1) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:mkdir, arglist"));
		FSmartbase_StringPtr(gCP,gTP,aArg1,&apStr,&apStrlen); // pathName argument
		aTmpBool = apDirObjectPtr->mkdir(QString(apStr));
		FrameExit(TBOOL(aTmpBool));
		break;
	case 29: //nameFilters
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:nameFilters, no dir object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:nameFilters, arglist"));
	...
	case 30: //new - Allocate C++ QtDir object
		...
		break;
	case 31: //path
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:path, no dir object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:path, arglist"));
		aTmpStr = apDirObjectPtr->path();
		*aResult = FSmartbase_CnvFromText(gCP,gTP,(char*)aTmpStr.toLatin1().data());
		FrameExit(*aResult);
		break;
	case 32: //refresh
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:refresh, no dir object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:refresh, arglist"));
		apDirObjectPtr->refresh();
		FrameExit(TBOOL(true));
		break;
	case 33: //remove
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:remove, no dir object"));
		if (apArgsLen != 1) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:remove, arglist"));
		FSmartbase_StringPtr(gCP,gTP,aArg1,&apStr,&apStrlen); // fileName argument
		aTmpBool = apDirObjectPtr->remove(QString(apStr));
		FrameExit(TBOOL(aTmpBool));
		break;
	case 34: //rename
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:rename, no dir object"));
		if (apArgsLen != 2) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:rename, arglist"));
		...
		break;
	case 35: //rmdir
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:rmdir, no dir object"));
		if (apArgsLen != 1) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:rmdir arglist"));
		FSmartbase_StringPtr(gCP, gTP, aArg1, &apStr, &apStrlen); // dirName argument
		aTmpBool = apDirObjectPtr->rmdir(QString(apStr));
		FrameExit(TBOOL(aTmpBool));
		break;
	case 36: //root
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:root, arglist"));
		aTmpDir = QDir::root();
		apDirObjectPtr = new QDir(aTmpDir.path());
		FrameExit(TFUNCTION(apDirObjectPtr));
		break;
	case 37: //rootDirPath
		FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:rootDirPath, obsolete"));
		break;
	case 38: //separator
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:separator, arglist"));
		aTmpStr = QDir::separator();
		*aResult = FSmartbase_CnvFromText(gCP, gTP, (char *)aTmpStr.toLatin1().data());
		FrameExit(*aResult);
		break;
	case 39: //setCurrent
		if (apArgsLen != 1) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:setCurrent arglist"));
		FSmartbase_StringPtr(gCP, gTP, aArg1, &apStr, &apStrlen); // path argument
		aTmpBool = QDir::setCurrent(QString(apStr));
		FrameExit(TBOOL(aTmpBool));
		break;
	case 40: //setFilter
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:setFilter, no dir object"));
		if (apArgsLen != 1) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:setFilter, arglist"));
		aTmpFilterSpec = (QDir::Filters)asInt(aArg1);
		apDirObjectPtr->setFilter(aTmpFilterSpec);
		FrameExit(TBOOL(true));
		break;
	case 41: //setMatchAllDirs
		FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:setMatchAllDirs, obsolete"));
		break;
	case 42: //setNameFilters
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:setNameFilters, no dir object"));
		if (apArgsLen != 1) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:setNameFilters, arglist"));
		FSmartbase_StringPtr(gCP, gTP, aArg1, &apStr, &apStrlen); // filter argument
		apDirObjectPtr->setNameFilters(QStringList(QString(apStr)));
		FrameExit(TBOOL(true));
		break;
	case 43: //setPath
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:setPath, no dir object"));
		if (apArgsLen != 1) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:setPath, arglist"));
		FSmartbase_StringPtr(gCP,gTP,aArg1,&apStr,&apStrlen); // path argument
		apDirObjectPtr->setPath(QString(apStr));
		FrameExit(TBOOL(true));
		break;
	case 44: //setSorting
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:setSorting, no dir object"));
		if (apArgsLen != 1) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:setSorting, arglist"));
		aTmpSortSpec = (QDir::SortFlags)asInt(aArg1);
		apDirObjectPtr->setSorting(aTmpSortSpec);
		FrameExit(TBOOL(true));
		break;
	case 45: //sorting
		if (apDirObjectPtr == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:sorting, no dir object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_Dir:sorting, arglist"));
...
TVAL RADGLUE_DisplayWorkbenchWindow(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	return(TERROR("!displayWorkbenchWindow not supported by RADGLUE!"));
}
...
TVAL RADGLUE_EncodeURL(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[])
{
	...
	char	aURLEncodeBuffer[RADGLUE_URL_BUFFER_SIZE];
	...
	*aTvWorkBuffer = FSmartbase_Eval(gCP, gTP, aStringFN, 1, *aTvWorkBuffer);
	...
	if (strlen((char*) FSmartbase_ObjectPtr(gCP, gTP, aTvWorkBuffer)) >= RADGLUE_URL_BUFFER_SIZE)
	...
	*aTvWorkBuffer = FSmartbase_CnvFromText(gCP, gTP, encodeUrl(aURLEncodeBuffer, RADGLUE_URL_BUFFER_SIZE));
	ExitOnError(*aTvWorkBuffer);	// Check for errors
...
integer as the following bit flags.  These flags are defined in aRADGLUE.h and in FSmartbase.h. They include:
RADGLUE_ERROR_TRACE			_FSmartbase_DEBUGON		- request for trace on error
RADGLUE_INSTRUCTION_TRACE	_FSmartbase_TRACEON		- request for instruction tracing
RADGLUE_SYSCHECKON			_FSmartbase_SYSCHECKON	- request for system checking
RADGLUE_JITON				_FSmartbase_JITON		- request for JIT processing
RADGLUE_ESCAPE				N/A						- request to stop processing
...
------------------------------------------------------------------*/
#define ENGINEFLAGSMASK RADGLUE_INSTRUCTION_TRACE | RADGLUE_ERROR_TRACE | RADGLUE_SYSCHECKON
NUM RADGLUE_Escape(LpXCONTEXT gCP, LpTHREAD gTP)
{
	long aResult = 0;
	long aContextIndex;
...
TVAL RADGLUE_EvalAMP(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	return(TERROR("!evalAMP not supported by RADGLUE!"));
}

/*-----------------------------------------------------------------
   RADGLUE_EvalsInSyncLocalContext
...
TVAL RADGLUE_FileInfo(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[])
{
	...
	if (argc < 2)
		FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo, arglist")); // make sure we have the minimum number of arguments
	apFileInfo = (QFileInfo *)asFunction(&argv[0]); // Get pointer to directory object if any
	aFunctionHandle = asInt(&argv[1]); // get apFunctionHandle for use in following dispatch table

	// Note that function arguments are passed in a single vector which is the third argument of the RADGLUE_FileInfo function.
	apArgsLen = 0; // Default to no arguments
	if (argc == 3)
	{	// Get Lambda arguments from arguments vector.
		*aArgs = argv[2];
		if (aArgs->Tag != TYVECTOR)
			FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo: bad arguments vector"));
		apArgsLen = FSmartbase_VectorLen(gCP,gTP,*aArgs);
		if (apArgsLen > 0) *aArg1 = FSmartbase_Ref(gCP,gTP,2,*aArgs,TINT(0));
		if (apArgsLen > 1) *aArg2 = FSmartbase_Ref(gCP,gTP,2,*aArgs,TINT(1));
		if (apArgsLen > 2) *aArg3 = FSmartbase_Ref(gCP,gTP,2,*aArgs,TINT(2));
	...
	{	// only valid for new call
		...
		*aArg3 = argv[4];
		...
		FrameExit(TERROR("!Error: RADGLUE_FileInfo - bad number of arguments"));
	switch(aFunctionHandle)
	{
	case 0: //_CleanUp - free resource allocated in the glue layer
		if (apFileInfo != 0)
	...
	case 1: //absoluteFilePath
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:absoluteFilePath, no fileInfo object"));
		if (apArgsLen != 0 ) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:absoluteFilePath, arglist"));
		aTmpStr = apFileInfo->absoluteFilePath();
		*aResult = FSmartbase_CnvFromText(gCP,gTP,(char*)aTmpStr.toLatin1().data());
		FrameExit(*aResult);
		break;
	case 2: //baseName
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:baseName, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:baseName, arglist"));
		apFileInfo->baseName();
		*aResult = FSmartbase_CnvFromText(gCP, gTP, (char*)aTmpStr.toLatin1().data());
		FrameExit(*aResult);
		break;
	case 3: //cachingPath
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:caching, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:caching, arglist"));
		aTmpBool = apFileInfo->caching();
		FrameExit(TBOOL(aTmpBool));
		break;
	case 4: //makeAbsolute
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:makeAbsolute, no fileInfo object"));
		if (apArgsLen != 0 ) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:makeAbsolute, arglist"));
		apFileInfo->makeAbsolute();
		FrameExit(TBOOL(true));
		break;
	case 5: //created
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:created, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:created, arglist"));
		...
		*aResult = FSmartbase_Eval(gCP,gTP,TGVALUE("date"),6,
		...
		break;
	case 6: //path
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:path, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:path, arglist"));
		aTmpDir = apFileInfo->path();
		apTmpDir = new QDir(aTmpDir.absolutePath());
		FrameExit(TFUNCTION(apTmpDir));
		break;
	case 7: //absolutePath
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:absolutePath, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:absolutePath, arglist"));
		aTmpStr = apFileInfo->absolutePath();
		*aResult = FSmartbase_CnvFromText(gCP,gTP,(char*)aTmpStr.toLatin1().data());
		FrameExit(*aResult);
		break;
	case 8: //exists
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:exists, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:exists, arglist"));
		aTmpBool = apFileInfo->exists();
		FrameExit(TBOOL(aTmpBool));
		break;
	case 9: //suffix
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:suffix, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:extension, arglist"));
		aTmpStr = apFileInfo->suffix();
		*aResult = FSmartbase_CnvFromText(gCP,gTP,(char*)aTmpStr.toLatin1().data());
		FrameExit(*aResult);
		break;
	case 10: // fileName
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:fileName, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:fileName, arglist"));
		aTmpStr = apFileInfo->fileName();
		*aResult = FSmartbase_CnvFromText(gCP,gTP,(char*)aTmpStr.toLatin1().data());
		FrameExit(*aResult);
		break;
	case 11: // filePath
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:filePath, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:filePath, arglist"));
		aTmpStr = apFileInfo->filePath();
		*aResult = FSmartbase_CnvFromText(gCP,gTP,(char*)aTmpStr.toLatin1().data());
		FrameExit(*aResult);
		break;
	case 12: //group
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:group, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:group, arglist"));
		aTmpStr = apFileInfo->group();
		*aResult = FSmartbase_CnvFromText(gCP,gTP,(char*)aTmpStr.toLatin1().data());
		FrameExit(*aResult);
		break;
	case 13: //groupId
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:groupId, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:groupId, arglist"));
		aTmpInt = apFileInfo->groupId();
		FrameExit(TINT(aTmpInt));
		break;
	case 14: //isDir
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:isDir, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:isDir, arglist"));
		aTmpBool = apFileInfo->isDir();
		FrameExit(TBOOL(aTmpBool));
		break;
	case 15: //isExecutable
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:isExecutable, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:isExecutable, arglist"));
		aTmpBool = apFileInfo->isExecutable();
		FrameExit(TBOOL(aTmpBool));
		break;
	case 16: //isFile
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:isFile, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:isFile, arglist"));
		aTmpBool = apFileInfo->isFile();
		FrameExit(TBOOL(aTmpBool));
		break;
	case 17: //isHidden
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:isHidden, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:isHidden, arglist"));
		aTmpBool = apFileInfo->isHidden();
		FrameExit(TBOOL(aTmpBool));
		break;
	case 18: //isReadable
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:isReadable, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:isReadable, arglist"));
		aTmpBool = apFileInfo->isReadable();
		FrameExit(TBOOL(aTmpBool));
		break;
	case 19: //isRelative
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:isRelative, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:isRelative, arglist"));
		aTmpBool = apFileInfo->isDir();
		FrameExit(TBOOL(aTmpBool));
		break;
	case 20: //isSymLink
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:isSymLink, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:isSymLink, arglist"));
		aTmpBool = apFileInfo->isSymLink();
		FrameExit(TBOOL(aTmpBool));
		break;
	case 21: //isWritable
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:isWritable, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:isWritable, arglist"));
		aTmpBool = apFileInfo->isWritable();
		FrameExit(TBOOL(aTmpBool));
		break;
	case 22: //lastModified
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:lastModified, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:lastModified, arglist"));
		...
		*aResult = FSmartbase_Eval(gCP,gTP,TGVALUE("date"), 6, TINT(aTmpDate.year()), TINT(aTmpDate.month()),
		...
		break;
	case 23: //lastRead
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:lastRead, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:lastRead, arglist"));
	...
	case 24: // new
		...
		{	// (_AISQtFileInfo #void aFunctionHandle #void #void fileInfoInstance) 
			...
			FrameExit(TERROR("RADGLUE_FileInfo:new, arglist"));			

		FrameExit(TFUNCTION(apFileInfo));
		break;
	case 25: //owner
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:owner, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:owner, arglist"));
		aTmpStr = apFileInfo->owner();
		*aResult = FSmartbase_CnvFromText(gCP,gTP,(char*)aTmpStr.toLatin1().data());
		FrameExit(*aResult);
		break;
	case 26: //ownerId
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:ownerId, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:ownerId, arglist"));
		aTmpInt = apFileInfo->ownerId();
		FrameExit(TINT(aTmpInt));
		break;
	case 27: //permission
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:refresh, no fileInfo object"));
		if (apArgsLen != 1) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:refresh, arglist"));
		aTmpBool = apFileInfo->permission((QFile::Permissions)asInt(aArg1));
		FrameExit(TBOOL(aTmpBool));			
		break;
	case 28: //readLink
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:readLink, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:readLink, arglist"));
	...
	case 29: //refresh
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:refresh, no fileInfo object"));
		if (apArgsLen != 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:refresh, arglist"));
		apFileInfo->refresh();
		FrameExit(TBOOL(true));			
		break;
	case 30: //setCaching
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:setCaching, no fileInfo object"));
		if (apArgsLen != 1) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:setFile, arglist"));
		apFileInfo->setCaching(asBool(aArg1));
		FrameExit(TBOOL(true));
		break;
	case 31: //setFile
		if (apFileInfo == 0) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:setFile, no fileInfo object"));
		if (apArgsLen != 1) FrameExit(TERROR((LpCHAR)"RADGLUE_FileInfo:setFile, arglist"));
		FSmartbase_StringPtr(gCP,gTP,aArg1,&apStr,&apStrlen); // FileName argument
...
datetime
------------------------------------------------------------------
TVAL RADGLUE_GetDirInfo(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
	char*	apBuf;
	long	aBufLen;
...
TVAL RADGLUE_GetTickCount(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	// Subtract the time when the program started(gStartClock initialized in RADGLUE_Init)
...
NUM RADGLUE_GetFileID(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
    // Unused Variables
    Q_UNUSED(gTP);
    Q_UNUSED(argc);
...
TVAL RADGLUE_HostCreate(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	return(TERROR("!hostCreate not supported by RADGLUE!"));
}
...
TVAL RADGLUE_HostDestroy(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	return(TERROR("!hostDestroy not supported by RADGLUE!"));
}
...
TVAL RADGLUE_HostGetProperty(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	return(TERROR("!hostGetProperty not supported by RADGLUE!"));
}
...
TVAL RADGLUE_HostInvoke(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	return(TERROR("!hostInvoke not supported by RADGLUE!"));
}
...
TVAL RADGLUE_HostPutProperty(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	return(TERROR("!hostPutProperty not supported by RADGLUE!"));
}
...
TVAL RADGLUE_HostSend(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	return(TERROR("!hostSend not supported by RADGLUE!"));
}
...
TVAL RADGLUE_Input(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	return(TERROR("!input not supported by RADGLUE!"));
}
...
TVAL RADGLUE_MsgBox(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	return(TERROR("!msgBox not supported by RADGLUE!"));
}
...
TVAL RADGLUE_NextUserID(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	return(TERROR("!nextUserID not supported by RADGLUE!"));
}
...
TVAL RADGLUE_Notify(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	return(TERROR("!notify not supported by RADGLUE!"));
}
...
/*-----------------------------------------------------------------
------------------------------------------------------------------*/
TVAL RADGLUE_test (LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[])
{
    // Unused Variables
    Q_UNUSED(argc);
...
TVAL RADGLUE_Select(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	return(TERROR("!select not supported by RADGLUE!"));
}
...
TVAL RADGLUE_ServerDoomed(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	return(TERROR("!serverDoomed not supported by RADGLUE!"));
}

/*-----------------------------------------------------------------
RADGLUE_SmOpenContext
...
/*-----------------------------------------------------------------
RADGLUE_Sleep
The RADGLUE_Sleep function calls a sleep on the currently executing 
thread.
------------------------------------------------------------------*/
TVAL RADGLUE_Sleep(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{ 
...
TVAL RADGLUE_Submit(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[])
{
	...
	FSmartbase_StringPtr(gCP, gTP, &argv[0], &apBuf, &aBufLen);	
	aRequestID = gpSessionManager->submit(gTP->SessionID, RADGLUE_EVENTCMD, 1 , apBuf, aDateTime, NULL/*Data*/);
	FrameExit(TINT(aRequestID));
}
...
/*-----------------------------------------------------------------------------------------------------------------------------
RADGLUE_UpdateState is registered in the context funcs structure as the call to _Host_UpdateState. This function is called each
					...
					NUM iMaxMemory, long iObjHdrSize, POINTER ipSessionMgrContextThread)
{
	...
	if (strlen(ipContextName) > 62)
		return -1;
	aFuncs._Host_Display = RADGLUE_Display;
	aFuncs._Host_Escape	= RADGLUE_Escape;
	aFuncs._Host_UpdateState = RADGLUE_UpdateState;
	aFuncs._Host_OpenF	= RADGLUE_Open;
	...
	gCP->ContextIndex = iContextIndex; // See RADGLUE.cpp gContextMutexs for usage
	...
	// (this uses the one and only catch environment)
	_FSmartbase_ECatch(gCP,/*out*/aCatchCode, LBLSystemThrowError);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"contextBusy",(LpFUNC)&RADGLUE_ContextBusy);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"contextClose",(LpFUNC)&RADGLUE_ContextClose);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"contextExists",(LpFUNC)&RADGLUE_ContextExists);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"contextOpen",(LpFUNC)&RADGLUE_ContextOpen);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"contextSubmit",(LpFUNC)&RADGLUE_ContextSubmit);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"contextStop",(LpFUNC)&RADGLUE_ContextStop);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"contextGetResult",(LpFUNC)&RADGLUE_ContextGetResult);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"contextGetDisplay",(LpFUNC)&RADGLUE_ContextGetDisplay);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"contextMyself",(LpFUNC)&RADGLUE_ContextMyself);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"contextSubscribe",(LpFUNC)&RADGLUE_ContextSubscribe);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"contextList",(LpFUNC)&RADGLUE_ContextList);

	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"ringBell",(LpFUNC)&RADGLUE_RingBell);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"debugDialog",(LpFUNC)&RADGLUE_DebugDialog);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"loadBrowseLambda",(LpFUNC)&RADGLUE_LoadBrowseLambda);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"loadUtilityLambdas",(LpFUNC)&RADGLUE_LoadUtilityLambdas);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"getTickCount",(LpFUNC)&RADGLUE_GetTickCount);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"sleep",(LpFUNC)&RADGLUE_Sleep); 
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"system",(LpFUNC)&RADGLUE_System);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"fileGetStatus",(LpFUNC)&RADGLUE_GetFileStatus);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"buildLambdaMessage",(LpFUNC)&RADGLUE_BuildLambdaMessage);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"decodeURL",(LpFUNC)&RADGLUE_DecodeURL);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"encodeURL",(LpFUNC)&RADGLUE_EncodeURL);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"evalsInSyncLocalContext",(LpFUNC)&RADGLUE_EvalsInSyncLocalContext);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"browse",(LpFUNC)&RADGLUE_Browse);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"input",(LpFUNC)&RADGLUE_Input);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"select",(LpFUNC)&RADGLUE_Select);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"hostCreate",(LpFUNC)&RADGLUE_HostCreate);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"hostSend",(LpFUNC)&RADGLUE_HostSend);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"hostInvoke",(LpFUNC)&RADGLUE_HostInvoke);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"hostDestroy",(LpFUNC)&RADGLUE_HostDestroy);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"hostGetProperty",(LpFUNC)&RADGLUE_HostGetProperty);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"hostPutProperty",(LpFUNC)&RADGLUE_HostPutProperty);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"notify",(LpFUNC)&RADGLUE_Notify);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"msgbox",(LpFUNC)&RADGLUE_MsgBox);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"nextUserId",(LpFUNC)&RADGLUE_NextUserID);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"evalAMP",(LpFUNC)&RADGLUE_EvalAMP);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"serverDoomed",(LpFUNC)&RADGLUE_ServerDoomed);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"displayWorkbenchWindow",(LpFUNC)&RADGLUE_DisplayWorkbenchWindow);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"openContext",(LpFUNC)&RADGLUE_SmOpenContext);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"readHtmlPage",(LpFUNC)&RADGLUE_ReadHtmlPage);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"getHttp",(LpFUNC)&RADGLUE_GetHttp);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"postHttp",(LpFUNC)&RADGLUE_PostHttp);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"sendToClient",(LpFUNC)&RADGLUE_SendToClient);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"enableConsoleLog",(LpFUNC)&RADGLUE_EnableConsoleLog);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"getConsoleLog",(LpFUNC)&RADGLUE_GetConsoleLog);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"openConsoleLog",(LpFUNC)&RADGLUE_OpenConsoleLog);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"throw",(LpFUNC)&RADGLUE_Throw);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"loadLib",(LpFUNC)&RADGLUE_LoadLib);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"reloadLib",(LpFUNC)&RADGLUE_ReloadLib);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"getOSDir",(LpFUNC)&RADGLUE_GetOSDir);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"mkOSDir",(LpFUNC)&RADGLUE_MkOSDir);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"cpOSFile",(LpFUNC)&RADGLUE_CpOSFile);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"existsOSFile",(LpFUNC)&RADGLUE_ExistsOSFile);
//	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"getMachineName",(LpFUNC)&RADGLUE_GetMachineName);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"_AISQDir",(LpFUNC)&RADGLUE_Dir);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"_AISQFileInfo",(LpFUNC)&RADGLUE_FileInfo);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"_AISContextClient",(LpFUNC)&RADGLUE_ContextClient);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"_AISTcpClient",(LpFUNC)&RADGLUE_TcpClient);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"submit",(LpFUNC)&RADGLUE_Submit);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"subscribe",(LpFUNC)&RADGLUE_Subscribe);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"getContextPtr",(LpFUNC)&RADGLUE_GetContextPtr);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"_aXml",(LpFUNC)&RADGLUE_AXml);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"createBuffer",(LpFUNC)&RADGLUE_CreateBuffer);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"deleteBuffer",(LpFUNC)&RADGLUE_DeleteBuffer);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"_test",(LpFUNC)&RADGLUE_test); // temporary testing stub 
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"openLog",(LpFUNC)&RADGLUE_OpenLog);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"startLog",(LpFUNC)&RADGLUE_StartLogging);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"stopLog",(LpFUNC)&RADGLUE_StopLogging);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"closeLog",(LpFUNC)&RADGLUE_CloseLog);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"md5hash",(LpFUNC)&RADGLUE_Md5Hash);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"addFileWatcher",(LpFUNC)&RADGLUE_AddToFileWatcher);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"removeFileWatcher",(LpFUNC)&RADGLUE_RemoveFromFileWatcher);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"sendFileChangedEvent",(LpFUNC)&RADGLUE_SendFileChangedEvent);
	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"sendStatusUpdateEvent",(LpFUNC)&RADGLUE_SendStatusUpdateEvent);
...
long errorNum(NUM iCode)
{
	switch (iCode)
    {
// case errFileRead:						return(RADGLUE_ERR_FILE_READ);
// case errFileWrite:						return(RADGLUE_ERR_FILE_WRITE);
    case FSMARTBASE_ERR_OUT_OF_MEMORY:		return(RADGLUE_ERR_OUT_OF_MEMORY);
    case FSMARTBASE_ERR_FRAME_ERROR:		return(RADGLUE_ERR_FRAME_ERROR);
    case FSMARTBASE_ERR_INVALID:			return(RADGLUE_ERR_INVALID);
    case FSMARTBASE_ERR_STACK:				return(RADGLUE_ERR_STACK);
    case FSMARTBASE_ERR_ESCAPE:				return(RADGLUE_ERR_ESCAPE);
    case FSMARTBASE_ERR_PCODE:				return(RADGLUE_ERR_PCODE);
	case FSMARTBASE_ERR_BAD_DATATYPE:		return(RADGLUE_ERR_BAD_DATATYPE);
    case FSMARTBASE_ERR_RECURSION:			return(RADGLUE_ERR_RECURSION);
    case FSMARTBASE_ERR_FRAME_RELEASE:		return(RADGLUE_ERR_FRAME_RELEASE);
	case FSMARTBASE_ERR_STACK_RELEASE:		return(RADGLUE_ERR_STACK_RELEASE);
    case FSMARTBASE_ERR_RECURSION_RELEASE:	return(RADGLUE_ERR_RECURSION_RELEASE);
	case FSMARTBASE_ERR_QUIT:				return(RADGLUE_ERR_QUIT);
    case FSMARTBASE_ERR_WRONG_VERSION:		return(RADGLUE_ERR_WRONG_VERSION);
    case FSMARTBASE_ERR_ENGINE_BUSY:		return(RADGLUE_ERR_ENGINE_BUSY);
    case FSMARTBASE_ERR_REPOSITORY_GC:		return(RADGLUE_ERR_REPOSITORY_GC);
	case FSMARTBASE_ERR_BADBUFFERSIZE:		return(RADGLUE_ERR_BADBUFFERSIZE);
    default:
		return iCode;
    }
}
...
//    case errFileWrite:				apMsg = "!errWriteIO!"; break;
	case RADGLUE_EVAL_FAILURE:			apMsg = "!evalFailure!";break;
	case RADGLUE_ERR_FILE_READ:			apMsg = "!errReadIO!";break;
	case RADGLUE_ERR_FILE_WRITE:			apMsg = "!errWriteIO!";break;
    case RADGLUE_ERR_OUT_OF_MEMORY:		apMsg = "!outOfMemory!"; break;
    case RADGLUE_ERR_FRAME_ERROR:		apMsg = "!gcFrameOverflw!"; break;
    case RADGLUE_ERR_INVALID:			apMsg = "!damagedMemoryHD!"; break;
    case RADGLUE_ERR_STACK:				apMsg = "!stackOverflw!"; break;
    case RADGLUE_ERR_ESCAPE:				apMsg = "!userEscape!"; break;
    case RADGLUE_ERR_PCODE:				apMsg = "!invalidPcode!"; break;
	case RADGLUE_ERR_BAD_DATATYPE:		apMsg = "!damagedObject!"; break;
    case RADGLUE_ERR_RECURSION:			apMsg = "!recursnLimit!"; break;
    case RADGLUE_ERR_FRAME_RELEASE:		apMsg = "!gcFrameRelease!"; break;
    case RADGLUE_ERR_STACK_RELEASE:		apMsg = "!stackRelease!"; break;
    case RADGLUE_ERR_RECURSION_RELEASE:	apMsg = "!recursnRelease!"; break;
	case RADGLUE_ERR_QUIT:				apMsg = "!userQuit!"; break;
    case RADGLUE_ERR_WRONG_VERSION:		apMsg = "!diskFileVersion!"; break;
    case RADGLUE_ERR_ENGINE_BUSY:		apMsg = "!engineBusy!"; break;
    case RADGLUE_ERR_REPOSITORY_GC:		apMsg = "!repoGC!"; break;
	case SGBLUE_UNEXPECTED_ERROR:		apMsg = "!unexpectedError!";break;
	case RADGLUE_ERR_NOT_IMPLEMENTED:	apMsg = "!notImplemented!"; break;
	case RADGLUE_ERR_SYSTEM_ERROR:		apMsg = "!systemError!"; break;
	case RADGLUE_ERR_EVAL_SECURITY:		apMsg = "!aSecurity!"; break;
	case RADGLUE_ERR_EVAL_BAD_CHECKIN:	apMsg = "!bad checkin argument!"; break;
	case RADGLUE_ERR_EVAL_BAD_FILTER:	apMsg = "!bad filter argument!"; break;
	case RADGLUE_ERR_EVAL_BAD_SCORE:		apMsg = "!bad score argument!"; break;
	case RADGLUE_SENDTOCLIENT_FAILURE:	apMsg = "!sendToClientFailure!";break;
	case RADGLUE_ERR_BADBUFFERSIZE:		apMsg = "!bad buffer size!";break;
    default:
...
// The following functions are Helper functions that are not registered by the engine and not exposed in RADGLUE.h. These
...
RADGLUE_SendStatusUpdateEvent

The RADGLUE_SendStatusUpdateEvent sends a status update to all associated sessions of the context
...
RADGLUE_CountOfOpenContexts

The CountOfOpenContexts returns the number of open contexts

#endif

long RADGLUE_CountOfOpenContexts()
{
...
TVAL RADGLUE_ContextOpen(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	*ec = FSmartbase_Setv(gCP, gTP, 3, &parms[0]);
	...
	//qDebug("Calling RADGLUE_ContextSubmit...");
	*tvalRequestId = RADGLUE_ContextSubmit(gCP, gTP, 2, &parms[0]);
...
TVAL RADGLUE_ContextSubmit(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[])
{
	...
	*tvalSessionId = FSmartbase_Refv(gCP, gTP, 2, &parms[0]);
	...
	aRequestID = gpSessionManager->submit(aSessionId, RADGLUE_EVENTCMD, 1 , lispCommand, aDateTime, NULL/*Data*/);
...
TVAL RADGLUE_ContextClose(LpXCONTEXT gCP, LpTHREAD gTP,NUM argc, TVAL argv[])
{
	...
	*tvalSessionId = FSmartbase_Refv(gCP, gTP, 2, &parms[0]);
	...
	//if (ipContext != NULL) 
	//{
	//	aRetcode = RADGLUE_FlushLispCodeInContext((void*)ipContext);
	//	if (aRetcode < 0)
	//	{
	//		*aResult = TERROR((LpCHAR)"!contextClose:error flushing Lisp resources in context!");
	//		FrameExit(*aResult);
	//	}
	//	// Must set mpCP to NULL so system will not crash next time RADGLUE_FlushLispCodeInContext is executed.
	//	apContext = gpSessionManager->getContextSt(aContextName);
	//	apContext->mpCP = NULL;
	//}
...
/*-----------------------------------------------------------------
RADGLUE_ContextList
Summary:	The RADGLUE_ContextList function returns a Vector of currently open context names. 
