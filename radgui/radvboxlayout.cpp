...
Constructor for the RadIde RadVBoxLayout. 
...
********************************************************************************************/
RadVBoxLayout::RadVBoxLayout()
	: QVBoxLayout(NULL)
{
	cpWrpVBoxLayout = new WrpVBoxLayout(this,(QWidget*)this,"WrpVBoxLayout");
}
...
Examples:
(addLayout: RadLayout))				;; Add the specified RadVBoxLayout or RadHBoxLayout to this RadVBoxLayout.
(addWidget: RadWidget))				;; Add the specified RadWidget to this RadVBoxLayout.
...
********************************************************************************************/
TVAL RadVBoxLayout::lisp(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc,TVAL argv[])
{
    ...
    *result = gCP->Tval_VOID;
	...
	{
		InvalidMessage:
		*result = TERROR("!RadVBoxLayout: first argument must be a valid message symbol!");
		goto Last;
	}
	...
	if (strcmp(messagePtr,"addLayout") == 0)
	{
		...
		{
			InvalidArgument:
			*ec = TERROR("!RadVBoxLayout.addLayout: 1st argument must be RadVBoxLayout or RadHBoxLayout!");
			FrameExit(*ec);
		}
	...
	else if (strcmp(messagePtr,"addWidget") == 0)
	{
		...
		if ((argc!=2)||(argv[1].Tag!=TYPOINTER))
		{
			*ec = TERROR("!RadVBoxLayout.addWidget: 1st argument must be RadWidget derived class!");
			FrameExit(*ec);
		}
	...
	else if (strcmp(messagePtr,"setStretchFactor") == 0)
	{
		...
		if ((argc<2)||(argv[1].Tag!=TYPOINTER))
		{
			*ec = TERROR("!RadVBoxLayout.setStretchFactor: 1st argument must be RadWidget/RadHBoxLayout/RadVBoxLayout derived class!");
			FrameExit(*ec);
		}
		else if ((argc<3)||(argv[2].Tag!=TYNUM))
		{
			*ec = TERROR("!RadVBoxLayout.setStretchFactor: 2nd argument must be an Integer stretch factor!");
			FrameExit(*ec);
		}
...
Lisp object creation method for the RadVBoxLayout class. Implements the RadIde RadVBoxLayout. 

Example:
	(setq radVBoxLayoutPtr (qt new: RadVBoxLayout:))	;; Create a new RadVBoxLayout object
					...
	(setq radVBoxLayoutPtr (qt delete: radVBoxLayoutPtr)) ;; Delete a radVBoxLayoutPtr object and release all of its memory.
...
********************************************************************************************/
TVAL RadVBoxLayout::lispNew(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc,TVAL argv[])
{
	RadVBoxLayout*	radVBoxLayoutPtr = new RadVBoxLayout();
	LpCHAR			textPtr;
	TVAL			manager;
	TVAL			result;
	...
	result.u.Pointer = (POINTER)radVBoxLayoutPtr->cpWrpVBoxLayout;
	return(result);
}
